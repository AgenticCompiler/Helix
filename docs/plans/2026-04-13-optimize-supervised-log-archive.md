...
**Goal:** Preserve supervised optimize orchestration logs by archiving `.helix` handoff artifacts into a persistent `optimize-logs/` directory while keeping the live `.helix/` runtime-only and temporary.

**Architecture:** Keep supervised optimize using the same live `.helix/` paths during execution, but add immutable per-round history snapshots plus a final archive copy under `optimize-logs/helix/<run-id>/` before cleanup removes the live runtime directory. Keep the behavior scoped to `--supervise on` so unsupervised optimize remains unchanged.
...
def test_prepare_exposes_history_and_archive_paths(self) -> None:
    ...
    self.assertEqual(state.history_dir, workdir / ".helix" / "history")
    self.assertEqual(state.archive_root, workdir / "optimize-logs" / "helix")
...
```bash
git add src/helix/optimize/guidance.py tests/test_optimize/guidance.py
...
def test_supervised_gate_writes_round_history_snapshots(self) -> None:
    ...
    self.assertTrue((workdir / ".helix" / "history" / "round-001-brief.md").exists())
    self.assertTrue(
        (workdir / ".helix" / "history" / "round-001-supervisor-report.md").exists()
    )
...
- keep writing the live `.helix/round-brief.md`
- keep writing the live `.helix/supervisor-report.md`
- additionally write immutable copies under `.helix/history/`
...
Run this step only for supervised optimize and only before live cleanup removes `.helix/`.
...
```bash
git add src/helix/optimize/guidance.py src/helix/optimize/orchestration.py tests/test_optimize/guidance.py tests/test_optimize_runtime.py
...
3. continue best-effort removal of live `.helix/` files
...
Document that `--supervise on` keeps live `.helix/` temporary but archives supervised orchestration artifacts under `optimize-logs/helix/<run-id>/` after the run.
...
```bash
git add README.md src/helix/optimize/guidance.py src/helix/optimize/orchestration.py tests/test_optimize/guidance.py tests/test_optimize_runtime.py docs/plans/2026-04-13-optimize-supervised-log-archive.md
