...
**Goal:** Add a repo-local Codex skill that helps create, list, and view GitCode pull requests with `gc pr`.

**Architecture:** Keep all behavior in a new `.codex/skills/managing-gitcode-prs/` skill folder. Put the durable workflow in `SKILL.md`, keep UI metadata in `agents/openai.yaml`, and move command detail into a small reference file so the trigger body stays concise.
...
Add a short design that fixes the scope to `gc pr create`, `gc pr list`, and `gc pr view`, requires `GC_TOKEN`, and keeps the skill repo-local under `.codex/skills/`.

- [ ] **Step 2: Review the design for scope drift**

Confirm the document does not expand into a general GitCode CLI skill and does not require changes under `src/`.
...
- when to use `pr create`, `pr list`, and `pr view`
- how to prefer current branch auto-detection but fall back to `--head`
- when to use `--fill`, `--draft`, `--json`, `--comments`, and `--web`
- how to report failures clearly when repo or branch detection is unavailable
...
Capture the supported `gc pr` command shapes and option notes in `references/pr-command-reference.md` so the main skill file can stay compact.
