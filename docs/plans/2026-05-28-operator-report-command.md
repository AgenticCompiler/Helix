...
Add a `helix report -i <operator_dir>` command that launches an AI agent
...
**File:** `src/helix/hardware_info.py` (new)
...
**File:** `src/helix/optimize/orchestration.py`
...
**File:** `src/helix/models.py` — 新增 `REPORT = "report"` + `COMMAND_TO_SKILL` 映射为空

**File:** `src/helix/cli.py` — 注册 `_CommandSpec`:
```python
...
**File:** `src/helix/skill_staging.py` — 新增 StageRule:
```python
...
**File:** `src/helix/prompts.py` — 新增 prompt intro:
```python
PROMPT_INTROS = {
    ...
...
**File:** `src/helix/commands/report.py` (new)
...
**File:** `src/helix/optimize/resume.py`
...
```bash
uv run helix report -i <operator_dir> --agent <agent>
```
...
**File:** `src/helix/commands/batch_report.py`
...
| `src/helix/hardware_info.py` | 新增 | W2.1: 硬件信息采集 |
| `src/helix/optimize/orchestration.py` | 修改 | W2.2: optimize 启动时写入 env-info.json |
...
| `src/helix/models.py` | 修改 | W4.1: REPORT command kind |
| `src/helix/cli.py` | 修改 | W4.1: 注册 report 命令 |
| `src/helix/skill_staging.py` | 修改 | W4.1: 新增 StageRule |
| `src/helix/prompts.py` | 修改 | W4.2: 新增 prompt intro |
| `src/helix/commands/report.py` | 新增 | W4.3: handler |
| `src/helix/optimize/resume.py` | 修改 | W5: reset 时删除 env-info.json |
| `src/helix/cli.py` | 修改 | W7.1: REPORT_BATCH 增加 has_agent |
| `src/helix/commands/batch_report.py` | 修改 | W7.2: 增强 handle_batch_report，per-workspace agent 启动 |
