...
**File:** `skills/triton-npu-optimize/references/opt-note-format.md`
...
**File:** `src/triton_agent/models.py` — 新增 `OPERATOR_REPORT = "report"` + `COMMAND_TO_SKILL` 映射为空
...
**File:** `src/triton_agent/commands/operator_report.py` (new)

```python
def handle_operator_report(parser, args) -> int:
    ...
    prompt = build_prompt(CommandKind.OPERATOR_REPORT, ...)
...
2. 对每个 workspace，执行 agent 生成 `report.md`：
   - 构建 AgentRequest：`command_kind=OPERATOR_REPORT`, `workdir=ws_dir`, `agent_name=...`
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--agent` | `codex` | 控制 per-workspace report 使用的 agent 后端 |
| `--show-output` | `false` | 是否实时输出 agent 的 stdout |
| `--report-workers` | `4` | 并发生成 report 的最大并行 worker 数 |

默认并行 4 个：使用 `concurrent.futures.ThreadPoolExecutor(max_workers=4)`，
...
| File | Change | Purpose |
|------|--------|---------|
| `skills/triton-npu-optimize/references/opt-note-format.md` | 修改 | W1: Overall Summary 模板增加 Key optimization points |
...
| `src/triton_agent/models.py` | 修改 | W4.1: OPERATOR_REPORT command kind |
...
| `src/triton_agent/commands/operator_report.py` | 新增 | W4.3: handler |
