...
**Goal:** Add `verify` to rerun correctness and benchmark validation for the numeric best optimize round in an isolated verification directory.

**Architecture:** Keep CLI parsing in `src/triton_agent/cli.py`, command handling in `src/triton_agent/commands/verification.py`, and feature-local resolution/execution in `src/triton_agent/verification/core.py`. Reuse baseline and round contract loaders plus the existing execution and compare helpers instead of duplicating test, benchmark, or perf parsing logic.
...
**Files:**
- Create: `docs/specs/2026-04-20-verify-design.md`
- Create: `docs/plans/2026-04-20-verify.md`
...
Run: `uv run python -m unittest tests.test_verify -v`

- [ ] **Step 3: Implement target resolution**

Add focused dataclasses and helpers in `verification/core.py`.
...
- [ ] **Step 1: Add `verify` to the command map**
...
Run: `uv run python -m unittest tests.test_verify tests.test_cli tests.test_execution_commands -v`
