...
**Architecture:** Keep CLI parsing in `src/helix/cli.py`, command handling in `src/helix/commands/verification.py`, and feature-local resolution/execution in `src/helix/verification/core.py`. Reuse baseline and round contract loaders plus the existing execution and compare helpers instead of duplicating test, benchmark, or perf parsing logic.
