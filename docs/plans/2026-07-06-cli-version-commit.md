...
**Goal:** Add top-level `triton-agent -v` and `triton-agent --version` support that prints only the build commit display value and exits successfully.

**Architecture:** Keep the CLI thin by wiring the standard argparse version action into the existing root parser. Reuse `triton_agent.build_info.get_build_info_display()` so version output matches the build-info value already shown in top-level help.
...
Expected: PASS with the updated `usage: triton-agent [-h] [-v] COMMAND ...` fixtures.
...
```bash
git add src/triton_agent/cli.py tests/test_cli.py tests/test_help_style.py
...
Run: `uv run python -m triton_agent.cli --version`
...
```bash
git add docs/specs/2026-07-06-cli-version-commit-design.md docs/plans/2026-07-06-cli-version-commit.md src/triton_agent/cli.py tests/test_cli.py tests/test_help_style.py
