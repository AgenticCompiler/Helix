...
Move optimize-specific prompt construction helpers out of `src/triton_agent/prompts.py` and into `src/triton_agent/optimize/prompts.py`.
...
- Keep `src/triton_agent/prompts.py` focused on shared prompt assembly.
...
Create a new module at `src/triton_agent/optimize/prompts.py` that owns:
...
Keep `src/triton_agent/prompts.py` as the shared prompt entry module. It should continue to own:
...
`build_prompt` should delegate optimize-specific sections to `triton_agent.optimize.prompts` instead of defining those helpers inline.

For compatibility, `src/triton_agent/prompts.py` may re-export optimize prompt helpers so existing tests and any internal callers do not break during this refactor.

## Expected Call-Site Changes

- Update optimize-local modules such as guidance, execution, and run-loop to import optimize prompt helpers from `triton_agent.optimize.prompts`.
- Leave shared callers on `triton_agent.prompts.build_prompt`.
