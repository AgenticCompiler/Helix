...
- `TRITON_AGENT_BATCH_WORKERS_PER_NPU` remains accepted for compatibility, but
  the managed MCP runtime ignores it when building its runtime device pool.

## Design

Update `src/triton_agent/run_eval_mcp_server.py` so the managed device pool is
...
- Add regression tests that verify `configured_slot_pool()` ignores
  `TRITON_AGENT_BATCH_WORKERS_PER_NPU` when devices are configured.
