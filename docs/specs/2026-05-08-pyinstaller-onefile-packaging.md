...
- Users run the executable directly from the platform artifact directory or from the generated zip archive.
...
- Generated zip archives contain only the onefile executable under the platform-tagged artifact directory. The archive writer must not recursively include stale files from an old onedir build directory.
