...
- Introduce a shared local benchmark profiler output variable named `TRITON_AGENT_BENCH_PROFILE_OUTPUT_DIR`.
...
- Local benchmark runners should first read `TRITON_AGENT_BENCH_PROFILE_OUTPUT_DIR`.
...
- Update local `msprof` benchmark coverage so the documented variable name is `TRITON_AGENT_BENCH_PROFILE_OUTPUT_DIR`.
