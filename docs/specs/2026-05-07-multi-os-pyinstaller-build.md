...
`helix` provides one repository script for creating a PyInstaller bundle on the current operating system. The same script can be run on Windows, Linux, and macOS, but each platform must build its own executable on that platform.
...
- The existing `packaging/helix.spec` remains the source of truth for bundled Python modules and the `skills/` data tree.
...
```text
dist/pyinstaller/helix-windows-x86_64/
dist/pyinstaller/helix-windows-x86_64.zip
```
