...
`helix` can be distributed as a PyInstaller `onedir` bundle that contains the Python CLI modules and the repository `skills/` tree. Users run the generated executable directly instead of invoking `uv run helix`.
...
- Add a central `helix.resources` module that resolves the application root from PyInstaller runtime metadata when frozen, and from the source repository root during normal development.
...
- Allow local generated-test and benchmark execution to use `HELIX_PYTHON`; when unset in a frozen executable, fall back to `python`.
