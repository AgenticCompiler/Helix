...
- Local `msprof` benchmarking also supports an opt-in artifact retention environment variable:
  - when `HELIX_BENCH_OUTPUT_DIR` is unset, behavior stays unchanged and temporary local profiler directories are deleted
  - when `HELIX_BENCH_OUTPUT_DIR` points to a directory, the local runner creates one preserved run directory under that location and stores each case under `case-<N>/`
...
- If `HELIX_BENCH_OUTPUT_DIR` points to a non-directory path, fail explicitly.
...
- Add unit coverage for local `msprof` benchmarking to verify:
  ...
  - setting `HELIX_BENCH_OUTPUT_DIR` preserves local per-case output directories under the configured root
