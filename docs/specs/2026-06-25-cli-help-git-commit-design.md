...
Extend `helix --help` so the top-level help output includes the CLI build's Git commit.

The design must support four runtime shapes with one consistent user-facing rule:

- source checkout execution such as `uv run helix --help`
...
- Show a Git commit identifier in top-level `helix --help`.
...
- The section includes one line:
  - `Git commit: <value>`
- `helix -h` and `helix --help` render the same build-info output.
...
- The build hook should accept a build-time environment override such as `HELIX_BUILD_GIT_COMMIT`.
...
- `build_py` should stage `helix/_build_meta.json` into the built package output.
- The sdist flow does not need to ship a pre-generated `helix/_build_meta.json` in the unpacked archive as long as downstream wheel builds can regenerate or preserve the same package-relative metadata through the packaged build hook.
...
- The PyInstaller packaging flow should include the same `helix/_build_meta.json` payload in the bundled application data.
- `packaging/helix.spec` should therefore add that package-relative metadata file to `a.datas`, alongside the existing bundled skills data.
...
- Add one narrow runtime helper module such as `src/helix/build_info.py`.
...
- This package-relative contract works for installed wheels and for PyInstaller when the bundled data is placed under the `helix/` package path.
- The existing frozen-runtime pattern in `src/helix/resources.py` is still a useful precedent, but the build-info lookup should not depend on `application_root()` alone because installed wheels need package-relative, not repository-root, resolution semantics.

## Failure Handling

- Missing or invalid Git metadata must not make `helix --help` fail.
- Missing or invalid embedded metadata must not make `helix --help` fail.
