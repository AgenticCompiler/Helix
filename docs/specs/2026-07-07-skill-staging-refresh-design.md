...
Change backend skill staging so a repeated `helix` launch refreshes the
...
- On a later launch, if `.<backend>/skills/<skill-name>` already exists and the
  current run wants to stage that skill again, `helix` removes that
...
`src/helix/skills/staging.py` should treat each selected staged skill
