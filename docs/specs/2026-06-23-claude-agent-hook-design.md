...
- `helix optimize --agent claude --enable-agent-hook` should stage temporary Claude hook files in the target workspace before launching Claude.
...
- Stage a temporary request-scoped Claude settings file under `.claude/helix-hooks/settings.json`.
...
- Stage these Claude-owned files:
  - `.claude/helix-hooks/settings.json`
  - `.claude/helix-hooks/policy.json`
  - `.claude/helix-hooks/pretooluse_guard.py`
  - `.claude/helix-hooks/tool_use_guard_policy.py`
