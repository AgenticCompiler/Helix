...
- Optimize workflow bootstrap and resume behavior should keep the same semantics
  ...
  - valid existing `.helix/state.json` is reused
...
- optimize runtime logic under `src/helix/optimize/`
- standalone-plugin-only Python copies under `python_support/`

That creates two structural problems:

1. The standalone Claude plugin depends on copied fragments of `helix`
...
This package must not import `helix`.

`helix` may import `hook_runtime` when the CLI wants to reuse the same
runtime logic.
...
- optimize workflow bootstrap and restore helpers currently rooted in
  `src/helix/optimize/workflow_state.py`
...
`helix.resources` or `helix.skill_catalog`.
...
plugin-specific fallback logic in `helix.resources` and
`helix.skill_catalog`.
...
- `python_support/`
- copied `helix` fragments for hook use

### CLI Reuse

The CLI should stop treating optimize workflow-state bootstrap as
`helix`-owned hook-private logic.

Instead:

- `src/helix/optimize/workflow_state.py` becomes a thin compatibility
...
- `helix` -> `hook_runtime` is allowed
- `hook_runtime` -> `helix` is not allowed

### Out Of Scope

This refactor does not need to unify the JavaScript hook guard in
`hooks/opencode/helix-hook-guard.js` with Python runtime modules.
...
- `src/helix/backends/claude_hooks.py`
- `src/helix/backends/codex_hooks.py`

### Deleted Or Removed From Runtime Contract

- `python_support/`
- hook-runtime fallbacks in `src/helix/resources.py`
- staged-flat fallback in `src/helix/skill_catalog.py`
