...
- Remove the redundant `src/helix/test_runner.py` and `src/helix/bench_runner.py` package wrappers.
- Keep execution-specific normalization in `src/helix/execution.py`.
...
- Keep optimize-only helper logic inside `src/helix/optimize/`.
...
- Update optimize resume logic to import metadata parsers from `src/helix/execution.py`.
