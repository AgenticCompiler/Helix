...
- Limit this change to prompt guidance and prompt tests for now; do not add new `optimize-check` enforcement in this iteration.
...
- Do not modify `optimize-check` or add static or runtime kernel continuity checks yet.
...
### Alternative: Add `optimize-check` Enforcement Immediately
...
- teaching `optimize-check` to reject rounds that preserve only output equivalence while bypassing the Triton kernel path
