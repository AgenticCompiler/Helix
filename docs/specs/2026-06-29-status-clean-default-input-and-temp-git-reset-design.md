...
- Cleanup should remove that temporary `.git` only when `TRITON_AGENT_RESET_GIT_REPO` is set.

## Goals

- Let users run `triton-agent status` and `triton-agent clean` directly inside a workspace.
...
- `uv run triton-agent status` behaves like `uv run triton-agent status --input .`
- `uv run triton-agent clean` behaves like `uv run triton-agent clean --input .`
- When skill staging creates `workdir/.git` for the current run, later cleanup keeps it by default.
- If `TRITON_AGENT_RESET_GIT_REPO` is set to a truthy value, cleanup removes only the temporary `.git` created by that run.
...
- Gate `SkillLinkManager.cleanup()` temporary-git removal on both:
  ...
  - `TRITON_AGENT_RESET_GIT_REPO` being enabled
...
- Skill cleanup coverage proving temporary `.git` is removed when `TRITON_AGENT_RESET_GIT_REPO` is enabled
