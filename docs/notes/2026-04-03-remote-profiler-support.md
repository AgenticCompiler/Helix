...
- Add remote-aware benchmark profiling through the unified `skills/triton-npu-run-eval/scripts/run-command.py` helper layer.
...
- Make `triton-npu-profile-operator` describe different argument rules for `standalone` and `msprof` benchmark modes.

## User-visible behavior

- Generated benchmark profiling should prefer:
  - `python3 skills/triton-npu-run-eval/scripts/run-command.py profile-bench --bench-file <bench> --operator-file <operator>`
...
- Keep remote SSH, `scp`, temporary workspace creation, and cleanup in the shared triton-npu-run-eval runtime so profiler execution follows the same semantics as existing remote test and benchmark helpers.
...
- Summaries should still be rendered by `skills/triton-npu-profile-operator/scripts/profile_summary.py` so profiler reporting stays centralized.
