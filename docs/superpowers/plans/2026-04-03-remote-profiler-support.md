...
**Goal:** Add a remote-aware profiling execution path so `ascend-npu-operator-profiler` can collect and summarize `msprof` output through the same remote execution model already used by validation and optimize workflows.

**Architecture:** Keep profiler orchestration in `skills/run-validation/scripts/` instead of teaching the profiler skill to manage SSH details itself. Add a new `profile-bench` helper entrypoint that resolves benchmark metadata, branches by `bench-mode`, reuses the existing remote workspace runtime helpers, copies the relevant profiler artifacts back to the local workspace, and then lets `ascend-npu-operator-profiler` summarize those local artifacts. Update the profiler skill so it documents the mode-specific benchmark contract and prefers the new helper over ad hoc `msprof <command>` invocations.

**Tech Stack:** Python 3.11, `argparse`, existing run-validation skill scripts, `unittest`, Markdown docs and skill guides
...
- [ ] **Step 5: Add failing contract tests that require `ascend-npu-operator-profiler` to document different argument rules for `standalone` and `msprof` benchmark modes**
...
**Files:**
- Modify: `skills/run-validation/scripts/run-command.py`
- Modify: `skills/run-validation/scripts/bench_runner.py`
- Modify: `skills/run-validation/scripts/run_runtime.py`
- Create: `skills/run-validation/scripts/profile_runner.py`
...
**Files:**
- Modify: `skills/ascend-npu-operator-profiler/SKILL.md`
- Modify: `skills/run-validation/SKILL.md`
- Modify: `skills/optimize/SKILL.md`

- [ ] **Step 1: Rewrite the profiler skill to prefer `../run-validation/scripts/run-command.py profile-bench` as the default execution path**
...
- [ ] **Step 6: Update the run-validation and optimize skill text so the new profiling helper is discoverable from the main execution path**
...
- Modify: `README.md`
- Modify: `AGENTS.md`
- Modify: `docs/2026-04-02-ascend-npu-operator-profiler-skill.md`
- Create: `docs/2026-04-03-remote-profiler-support.md`

- [ ] **Step 1: Write a short design doc that explains why profiler remote support belongs in the unified run-validation runtime instead of inside the profiler skill**
...
**Files:**
- Modify: `skills/run-validation/scripts/run-command.py`
- Modify: `skills/run-validation/scripts/profile_runner.py`
- Modify: `skills/ascend-npu-operator-profiler/SKILL.md`
- Modify: `README.md`
