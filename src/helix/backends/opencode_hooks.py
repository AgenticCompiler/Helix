...
from helix.backends.hook_common import HookStageOptions, HookStageState, cleanup_hook_stage


_OPENCODE_HOOK_DIR = Path(".opencode") / "helix-hooks"
_OPENCODE_PLUGIN_FILE = Path(".opencode") / "plugins" / "helix-hook-guard.js"
_OPENCODE_DENY_MESSAGE = (
    ...
    "staged skill implementation files under .opencode/skills/*/scripts/, or helix-logs/ "
    "output). "
...
_OPENCODE_DENY_READ_GLOBS = _SHARED_DENY_READ_GLOBS + (
    Path(".opencode") / "plugins" / "helix-hook-guard.js",
    Path(".opencode") / "helix-hooks",
    Path(".opencode") / "helix-hooks" / "**",
...
def prepare_opencode_hooks(
    ...
    *,
    ...
    try:
        ...
        shutil.copy2(template_dir / "helix-hook-guard.js", plugin_file)
