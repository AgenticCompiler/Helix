...
def run_buffered_process(
    ...
    owns_process_group = interrupt_policy is not None and not _IS_WINDOWS
    ...
    try:
        while True:
            ...
            if stall_timeout_seconds > 0 and time.monotonic() - start_ref[0] > stall_timeout_seconds:
                _terminate_process_tree(process, grace_seconds=1.0, owns_process_group=owns_process_group)
                for reader in readers:
                    reader.join(timeout=1.0)
    ...
    _reap_process_tree_after_exit(process, owns_process_group=owns_process_group)
    session_id = _flush_session_id_buffer(session_id_extractor, session_id, session_buffer)
...
def _run_streaming_process_windows(
    ...
    *,
    ...
    try:
        while True:
            ...
            if stall_timeout_seconds > 0 and elapsed > stall_timeout_seconds:
                _terminate_process_tree(process, grace_seconds=1.0, owns_process_group=False)
                stalled_ref[0] = True
                break
    except KeyboardInterrupt:
        if interrupt_policy is None:
    ...
    if not stalled_ref[0]:
        _reap_process_tree_after_exit(process, owns_process_group=False)
...
def _terminate_process_tree(
    ...
    *,
    ...
    if _IS_WINDOWS or not owns_process_group:
        process.terminate()
...
def _reap_process_tree_after_exit(process: subprocess.Popen[Any], *, owns_process_group: bool) -> None:
    if _IS_WINDOWS or not owns_process_group:
        return
