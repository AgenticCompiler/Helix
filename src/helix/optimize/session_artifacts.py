...
from helix.optimize.memory_file import MemoryFileManager, MemoryFileState
from helix.optimize.profile_cleanup import cleanup_optimize_workspace_profile_artifacts
from helix.optimize.subagents import perf_diagnosis_subagent_definition
from helix.optimize.workflow_state import (
...
class OptimizeSessionArtifactsManager:
    ...
    def prepare_supervised_session(
        ...
        supervisor_handoff_dir = hidden_helix_dir / "supervisor-handoffs"
    ...
    def archive(self, state: OptimizeSessionArtifactsState) -> list[str]:
        ...
        warnings = self._archives.archive(
            state.archive,
    ...
    def describe_cleanup_supervised_session(
        ...
        if state.hidden_helix_dir is not None:
            messages.append(
    ...
    def _prepare_hidden_helix_dir(self, workdir: Path) -> Path:
        hidden_helix_dir = workdir / ".helix"
        if hidden_helix_dir.exists() and any(hidden_helix_dir.iterdir()):
            raise RuntimeError(
    ...
    def _cleanup_hidden_helix_dir(self, hidden_helix_dir: Path | None) -> list[str]:
        if hidden_helix_dir is None:
            return []
        warnings: list[str] = []
        if hidden_helix_dir.name != ".helix":
            return [
