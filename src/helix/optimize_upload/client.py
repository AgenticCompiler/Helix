...
from helix.optimize_upload.models import UploadIdentity, UploadResponse


class UploadUrlMissingError(ValueError):
    """Raised when HELIX_OPTIMIZE_UPLOAD_URL is not set."""

    def __init__(self) -> None:
        super().__init__(
            "HELIX_OPTIMIZE_UPLOAD_URL is not set. "
...
def load_upload_url() -> str:
    url = os.environ.get("HELIX_OPTIMIZE_UPLOAD_URL")
    if not url:
...
def upload_tarball(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "X-Helix-Upload-Uid": identity.upload_uid,
            "X-Helix-Upload-Timestamp": identity.upload_timestamp,
            "X-Helix-Workspace-Name": identity.workspace_name,
            "X-Helix-Workspace-Slug": identity.workspace_slug,
