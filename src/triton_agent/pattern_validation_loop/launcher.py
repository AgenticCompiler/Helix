...
Staged optimize knowledge to edit during the loop:
...
2. Read `{synthesis_path.as_posix()}` (and `PERF_KNOWLEDGE_BASE.md` if needed). **You** decide which lessons become staged pattern card edits under `{knowledge_root.as_posix()}/references/patterns/`.
...
5. Run from `{repo_path.as_posix()}`:
   ...
   triton-agent optimize-batch -i {batch_dir.as_posix()} --resume fresh --reset-optimize --min-rounds {min_rounds} --max-concurrency 1 --show-output --optimize-knowledge {optimize_knowledge} --agent {agent_name}
...
7. If any **active** workspace fails audit and iteration < {max_iterations}:
   ...
   - update staged pattern cards + regenerate index
   - run `{scripts_dir.as_posix()}/reset_workspace_rounds.py --batch-root {batch_dir.as_posix()}`
   - re-run optimize-batch with `--resume continue` per workspace or the whole batch as appropriate
   - re-audit
    ...
    *,
    ...
    if not synthesis_path.is_file():
        raise ValueError(
            ...
            "Run analyze-commit-perf first or pass --synthesis to an existing report.",
        )
