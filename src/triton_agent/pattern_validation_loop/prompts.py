...
def build_prepare_prompt(
   ...
   - Use `knowledge_lessons`, `launch_functions`, and `kernels_in_operator` from each entry when writing batch evaluation records (step 7).
...
5. Scaffold workspaces under `{batch_dir.as_posix()}`: **workspace-named** directories and operator files per the plan; helper `.py` only under `deps/`.
   ...
   - Copy repo `test_*.py` as **reference** files named `test_*.py.txt` (not runnable pytest). Record `copied_tests` in the batch registry (step 7).
   - Do **not** write `validation-meta.json` inside workspace directories.
...
7. Write **one** batch-level registry `{batch_dir.as_posix()}/batch-evaluation.json` (do not put ground truth inside workspaces):

   python3 {scripts_dir.as_posix()}/batch_evaluation.py --help

   For each workspace directory, upsert `workspaces.<name>` with at least:
   `workspace`, `source_path`, `operator_filename`, `launch_functions`, `kernels_in_operator`,
   `expected_patterns`, `dependency_dir`, `copied_tests`, `copied_dependencies`, and notes.
   Use `upsert_workspace_entry` from `batch_evaluation.py` or write JSON consistent with that module.
{record_step}
...
artifacts inside each workspace directory before deciding next steps.
