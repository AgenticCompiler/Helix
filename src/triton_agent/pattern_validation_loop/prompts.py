...
def build_prepare_prompt(
   ...
   - One entry in `workspaces[]` → one directory `{batch_dir.as_posix()}/<kernel_name>/` with operator `<kernel_name>.py`.
...
{plan_step}
5. Scaffold workspaces under `{batch_dir.as_posix()}`: **kernel-named** directories and operator files per the plan; helper `.py` only under `deps/`.
