...
def load_baseline_state(workspace: Path) -> BaselineState:
    return _OPTIMIZE_CHECK_MODULE.load_baseline_state(workspace)


def inspect_baseline_artifacts(workspace: Path) -> BaselineArtifactsInspection:
    return _OPTIMIZE_CHECK_MODULE.inspect_baseline_artifacts(workspace)


def baseline_gate_issues(workspace: Path) -> tuple[str, ...]:
    return _OPTIMIZE_CHECK_MODULE.baseline_gate_issues(workspace)
