...
def _resolve(text: str, *, language: str) -> str:
    """Replace {language} and {Language} placeholders in prompt text."""
    display = _LANGUAGE_DISPLAY.get(language, language.capitalize())
    return text.replace("{language}", language).replace("{Language}", display)
...
def strict_learned_lessons_lines(*, language: str) -> list[str]:
    return [
        _resolve(
            "`learned_lessons.md` is only for reusable, evidence-backed optimization or "
            "profiling rules that can transfer to related {Language} Ascend NPU operators.",
            language=language,
        ),
...
def next_round_reflection_lines() -> list[str]:
    return [
        "Use the staged `npu-optimize-start-round` skill before opening the next round.",
...
def layered_analysis_lines(*, round_scope: str) -> list[str]:
    return [
        ...
        "Use the staged `{language}-npu-optimize-knowledge` skill for generic pattern and symptom references.",
        ...
        "Read the staged `{language}-npu-optimize-knowledge` skill's generated `references/pattern_index.md` before detailed pattern references.",
...
def compiler_source_analysis_lines(
    *,
    ...
    return [
        ...
        "Use the staged `{language}-npu-analyze-compiler-source` skill only when compiler source evidence is needed.",
...
def cann_ext_api_lines(*, enabled: bool, language: str) -> list[str]:
    if not enabled:
        return []
    return [
        _resolve(
            "CANN {Language} extension API pattern access is enabled for this optimize run.",
            language=language,
        ),
...
def optimize_target_lines(*, optimize_target: str, language: str) -> list[str]:
    if optimize_target == "operator":
        return [
            ...
            _resolve(
                "Preserve a real {Language} Ascend NPU computation path.",
                language=language,
            ),
            _resolve(
                "A pure PyTorch rewrite that bypasses the {Language} Ascend NPU path "
                "does not count as a successful optimize round.",
                language=language,
            ),
    ...
    return [
        ...
        _resolve(
            "You must continue optimizing the {Language} Ascend NPU kernel path itself.",
            language=language,
        ),
        ...
        _resolve(
            "A round that bypasses the {Language} kernel path with pure PyTorch code "
            "does not count as a successful optimize round.",
            language=language,
        ),
    ...
    *,
    ...
    return [
        *optimize_target_lines(optimize_target=optimize_target, language=language),
        "Use the staged `npu-prepare-optimize-baseline` skill when baseline artifacts are missing or invalid.",
        optimize_check_line,
        *(
        ...
        "If baseline preparation is needed, use the staged `npu-prepare-optimize-baseline` skill and continue only after it has repaired the baseline through `npu-optimize-submit-baseline`.",
        "For each round, write the optimized operator snapshot as `opt_<original-operator>.py` inside `opt-round-N/`.",
        "For each round, keep the benchmark artifact as `opt_<original-operator>_perf.txt` inside `opt-round-N/`, ensure that file is generated by the `npu-run-eval` skill's `run-bench` flow, and record that exact filename in `round-state.json`.",
        ...
        "Use the staged `npu-analyze-round-performance` skill when a round needs deeper diagnosis from profile or IR evidence.",
        "When you use that analysis flow, write `opt-round-N/perf-analysis.md` as the standalone analysis artifact.",
        "Use `{language}-npu-analyze-ir` as the IR evidence companion when IR attribution is needed, while `npu-analyze-round-performance` remains the owner of `opt-round-N/perf-analysis.md`.",
    ...
    if latest_round_dir is not None:
        ...
        lines.extend(
            [
                ...
                "Read the staged `{language}-npu-optimize`, `npu-prepare-optimize-baseline`, `npu-optimize-submit-baseline`, `npu-optimize-submit-round`, and `npu-optimize-start-round` skills as the workflow contract that the worker round was supposed to follow.",
                ...
                _resolve(
                    "Reject rounds that preserve only the public API shape but replace "
                    "the {Language} kernel path with pure PyTorch computation.",
                    language=language,
                ),
    ...
    continuation_lines = [
        ...
        (
            ...
            if optimize_target == "operator"
                ...
                "Continue optimizing the {Language} Ascend NPU kernel path itself.",
                language=language,
            )
        ),
    ...
    *,
    ...
    lines.extend(
        _shared_optimize_prompt_lines(
            ...
            optimize_check_line=(
                "You must run the staged `npu-optimize-submit-round` skill after each completed round."
            ),
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "Use the staged `npu-optimize-submit-baseline` skill to submit the baseline until it passes.",
