...
def run_local_bench(
    ...
    if bench_mode == "msprof":
        return _run_local_bench_msprof(
            bench_file,
            operator_file,
        )
    return _run_local_bench_standalone(
        bench_file,
        operator_file,
    )
...
def _run_local_bench_standalone(
    ...
    result = run_process(command, str(bench_file.parent), mode="streaming", stall_timeout_seconds=900)
...
def _perf_output_path(bench_file: Path, operator_file: Path) -> Path:
    filename = f"{operator_file.stem}_perf.txt"
    return operator_file.parent / filename
