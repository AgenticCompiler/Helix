...
_COMMAND_SPECS: dict[CommandKind, _CommandSpec] = {
    ...
    CommandKind.PATTERN_VALIDATION_SIMULATE: _CommandSpec(
        ...
        help_group="Optimization",
        help_summary="Extract commit perf knowledge and embed patterns into skills (dry-run loop).",
        description=(
            "End-to-end loop: commit performance extraction (when PERF reports are missing), "
            "workspace plan, prepare agent (when batch is empty), verify, simulate agents per "
            "workspace, skill-audit on pattern-validation-skills; repeat until aligned or "
            "--max-iterations. Real optimize-batch only with --run-optimize."
        ),
        has_output=False,
        has_agent=True,
        has_show_output=True,
    ...
    for command_kind in CommandKind:
        ...
        if spec.has_pattern_validation_simulate_options:
            subparser.add_argument(
                ...
                default="PERF_PATTERN_SYNTHESIS.md",
                help=(
                    "Synthesis report path (default: PERF_PATTERN_SYNTHESIS.md). "
                    "Generated by commit extraction when missing."
                ),
            )
            subparser.add_argument(
                "--knowledge-base",
                default="PERF_KNOWLEDGE_BASE.md",
                help=(
                    "Incremental knowledge base path (default: PERF_KNOWLEDGE_BASE.md). "
                    "Generated by commit extraction when missing; also drives workspace planning."
                ),
            )
            subparser.add_argument(
                "--base",
                default="origin/main",
                help=(
                    "Base revision for commit extraction and workspace-plan generation "
                    "(default: origin/main)."
                ),
            )
            subparser.add_argument(
                ...
                action="store_true",
                help=(
                    "Skip commit performance extraction even when PERF reports are missing. "
                    "Use only when synthesis and knowledge base already exist."
                ),
            )
            subparser.add_argument(
                "--include-ir",
                action="store_true",
                help=(
                    "During commit extraction, stage IR analysis support and ask the agent "
                    "to include IR evidence when available."
                ),
            )
            subparser.add_argument(
                "--force",
                action="store_true",
                help=(
                    "During commit extraction, overwrite existing incremental and synthesis "
                    "PERF reports if they already exist."
                ),
            )
            subparser.add_argument(
                "--skip-launch",
            ...
            subparser.add_argument(
                ...
                action="store_true",
                help=(
                    "Do not launch the prepare agent when the batch has no workspaces. "
                    "Default: auto-run prepare (plan is already generated by the CLI)."
                ),
            )
            subparser.add_argument(
                "--run-optimize",
            ...
            subparser.add_argument(
                "--concurrency",
                type=_parse_positive_int_value,
                default=1,
                help=(
                    "Maximum number of simulate agents to run in parallel per iteration "
                    "(default: 1)."
                ),
            )
