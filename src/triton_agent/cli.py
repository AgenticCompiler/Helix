...
_COMMAND_SPECS: dict[CommandKind, _CommandSpec] = {
    ...
    CommandKind.PATTERN_VALIDATION_SIMULATE: _CommandSpec(
        ...
        description=(
            "End-to-end dry-run loop: workspace plan, prepare agent (when batch is empty), "
            "verify, simulate agents per workspace, skill-audit on pattern-validation-skills; "
            "repeat until aligned or --max-iterations. Real optimize-batch only with --run-optimize."
        ),
        has_output=False,
        has_agent=True,
        has_show_output=True,
