...
- When modifying `skills/common/ascend-npu-optimize-state/references/baseline-contract.json` or `skills/common/ascend-npu-optimize-state/references/round-contract.json`, rerun `uv run python -m triton_agent.optimize_knowledge.update_artifacts` so `skills/triton/triton-npu-optimize/references/artifacts.md` stays in sync.
