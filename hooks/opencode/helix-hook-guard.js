...
const PROTECTED_RELATIVE_PATH_PREFIXES = [".helix/", "helix-logs/"];
const WINDOWS_ABSOLUTE_PATH_RE = /^[A-Za-z]:[\\/].+/;
const EDIT_TOOLS = new Set(["edit", "write", "patch", "update", "multiedit", "multi_edit"]);
const WORKFLOW_STATE_RELATIVE_PATH = ".helix/state.json";
...
export async function HelixHookGuard(context) {
...
async function loadPolicy(context) {
  for (const root of policySearchRoots(context)) {
    try {
      const policyPath = path.join(root, ".opencode", "helix-hooks", "policy.json");
...
async function denyReasonForToolUse(policy, input, output) {
  ...
  // .helix/state.json for phase enforcement before falling
...
function isProtectedRuntimeEditPath(relativePath) {
  if (relativePath === ".helix" || relativePath.startsWith(".helix/")) {
    return true;
  }
  if (relativePath === "helix-logs" || relativePath.startsWith("helix-logs/")) {
    return true;
  }
  ...
  if (relativePath.startsWith(".codex/helix-hooks/")) {
    return true;
  }
  if (relativePath.startsWith(".claude/helix-hooks/")) {
    return true;
  }
  if (relativePath.startsWith(".opencode/helix-hooks/")) {
    return true;
  }
  return false;
}
