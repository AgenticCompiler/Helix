...
class PiRunnerTests(unittest.TestCase):
    def test_non_interactive_command_uses_print_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(command[skill_index + 1], str(workspace / ".pi" / "skills"))
    ...
    def test_optimize_no_agent_session_adds_no_session(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            runner = PiRunner()
            request = AgentRequest(
    ...
    def test_run_uses_unified_process_runner(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("triton_agent.backends.pi.run_process", return_value=_ok_result()) as mocked:
                runner.run(request)
    ...
    def test_verbose_logging_prints_launch_command(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stderr = StringIO()
            with patch("triton_agent.backends.pi.run_process", return_value=_ok_result()):
            ...
            self.assertIn("--no-session", stderr.getvalue())
            self.assertIn("--skill", stderr.getvalue())
