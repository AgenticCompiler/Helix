...
class CompilerSourceTests(unittest.TestCase):
    ...
    def test_prepare_reuses_existing_checkout_without_fetch_or_pull(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            checkout = Path(tmp) / "AscendNPU-IR"
            checkout.mkdir()
            (checkout / ".git").mkdir()
            ...
            def fake_run(args: list[str], cwd: Optional[Path] = None) -> str:
                self.assertEqual(cwd, resolved_checkout)
                calls.append(args)
                if args == ["git", "rev-parse", "HEAD"]:
                    return "def456\n"
                if args == ["git", "status", "--porcelain"]:
            ...
            self.assertEqual(
                result,
                CompilerSourceInfo(path=resolved_checkout, commit="def456", dirty=True),
            )
    ...
    def test_prepare_rejects_file_checkout_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "AscendNPU-IR"
            path.write_text("not a directory\n", encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "not a directory"):
                prepare_compiler_source(mode="auto", source_path=path)

    def test_prepare_rejects_missing_explicit_checkout_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
