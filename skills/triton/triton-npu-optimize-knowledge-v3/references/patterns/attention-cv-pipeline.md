...
For example, when targeting A5-family devices such as `ascend950PR` or `ascend950DT`, keep compile-parameter gates explicit rather than global defaults.
...
- Record architecture-gated decisions and outcomes in `attempts.md` / `summary.md` so later rounds do not silently drop target constraints.
