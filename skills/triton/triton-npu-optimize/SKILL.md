...
- **Structural optimization priority gate (MUST evaluate before any micro-optimizations):** Before considering fp32-elision, contiguous-call removal, cache modifiers, or autotune, you **must** evaluate these six structural optimization directions in order. Record the evaluation result for each in `attempts.md` even if the answer is "not applicable":
  ...
  5. **Atomic-contention owner-computes (atomic-contention-owner-computes-store):** Does the kernel use `tl.atomic_add`, `tl.atomic_max`, or similar atomic operations where many programs write to a small output domain (e.g., per-row sums, per-channel statistics, histogram bins, segment reductions)? If yes, you **must** read [`../triton-npu-optimize-knowledge/references/patterns/atomic-contention-owner-computes-store.md`](../triton-npu-optimize-knowledge/references/patterns/atomic-contention-owner-computes-store.md) and evaluate whether transposing the grid to output-ownership (one program per output target, scanning the contributing input range) can eliminate atomic serialization. This applies to **any** kernel with `tl.atomic_*` targeting a moderate-cardinality output domain, including normalization moments kernels (GroupNorm/InstanceNorm/LayerNorm/BatchNorm backward), scatter-reduce, histogram, and segment-sum. **Key insight for normalization operators:** per-channel or per-group program decomposition (each program owns one channel/group, scans the spatial dimension with SPATIAL_BLOCK tiling) aligns the kernel structure with the operator's reduction axis, eliminating atomic contention and improving memory access patterns.
  6. **Wrapper buffer reuse (wrapper-buffer-reuse) — fallback when fusion is blocked:** Has auxiliary-op fusion been attempted and blocked by correctness, precision, or compile constraints (e.g., fp16 Inf mismatch)? If yes, and the torch wrapper chain still dominates `total_op_avg_time_us` with multiple aclnn ops each allocating a temporary tensor, you **must** read [`../triton-npu-optimize-knowledge/references/patterns/wrapper-buffer-reuse.md`](../triton-npu-optimize-knowledge/references/patterns/wrapper-buffer-reuse.md) and evaluate pre-allocated buffer reuse via the `out=` parameter, in-place `+=` accumulation, and an in-place pass-through kernel to eliminate the output copy. This is a **fallback** direction — do not choose it before attempting fusion. Record the specific fusion blocker that forced this fallback in `attempts.md`.
- After the structural optimization priority gate is satisfied (all six directions evaluated and addressed where applicable), proceed to the mandatory diagnostic steps below.
- **Compilation-failure diagnostic gate (bounded fix-then-revert):** When a round's code change causes a Triton compilation error, do NOT immediately revert or abandon the direction. But do NOT get stuck endlessly debugging a compilation error either. Bounded protocol: (1) read the error carefully; (2) attempt **at most 2** targeted fixes addressing the root cause (examples: dtype mismatch on block-pointer store → `val.to(ptr.dtype.element_ty)`, shape/stride error in `tl.make_block_ptr`, missing `tl.constexpr`, `tl.range` vs `tl.static_range` type mismatch); (3) record the error and each fix attempt in `attempts.md`; (4) after at most 2 failed fix attempts, or immediately if the error is clearly unresolvable (unsupported API on Ascend NPU, fundamental architectural conflict), **revert to the parent code and continue to the next optimization direction**. A compilation error is a diagnostic signal, not a dead end — but it is also not a research project. The agent must attempt a fix, but also know when to stop.
- **Correctness-failure from precision divergence (bounded fix-then-fallback):** When a fusion or kernel-rewrite attempt fails correctness due to fp16/bf16 precision divergence (Inf/NaN location mismatch, overflow behavior difference between Triton and aclnn arithmetic), apply the same bounded protocol: (1) attempt **at most 2** targeted fixes (examples: replace `* reciprocal` with `/ divisor` to match aclnn division semantics, reorder arithmetic to match aclnn associativity, use fp32 intermediate accumulation); (2) record the specific precision blocker in `attempts.md`; (3) after at most 2 failed fix attempts, **abandon the fusion/kernel-rewrite direction for this round** and fall back to the [`../triton-npu-optimize-knowledge/references/patterns/wrapper-buffer-reuse.md`](../triton-npu-optimize-knowledge/references/patterns/wrapper-buffer-reuse.md) pattern — pre-allocated buffer reuse via `out=` parameter, in-place `+=` accumulation, and an in-place pass-through kernel. The wrapper-level optimization does not change aclnn arithmetic and cannot introduce precision regressions, making it the safe fallback when precision blocks kernel-level changes. Do NOT spend an entire round debugging fp16 Inf propagation — the wrapper-level optimization surface is always available and safe.
- **Pattern testing methodology — test on the highest-overhead variant first:** When a pattern is applied to a kernel that dispatches to multiple code paths (e.g., masked + no-mask exact-tile, general + single-pass), apply the pattern to the **highest-overhead variant first** — the one with masks, boundary checks, address computation loops, or scalar-heavy dispatch logic. Do NOT test on an already-optimized no-mask fast path, observe no improvement, and then reject the pattern for all variants. The no-mask kernel has minimal optimization headroom; gains there are the smallest. This applies to ALL patterns, not just specific ones — compile hints, cache modifiers, reorder-load, block pointers, etc. all produce the largest measurable impact on the variant with the most scalar/transfer overhead. If the highest-overhead variant shows improvement, propagate to other variants; only reject the pattern entirely if the highest-overhead variant shows no gain.
...
- When a kernel uses `tl.atomic_add` or `tl.atomic_max` to update a small-to-moderate output domain (per-row sums, per-channel statistics, histogram bins, segment reductions) from many programs, evaluate the `atomic-contention-owner-computes-store` pattern: transpose the grid so each program owns one output target and scans its contributing input range. This eliminates atomic serialization and is often the single highest-impact structural change for scatter-reduce kernels including GroupNorm/InstanceNorm/LayerNorm moments kernels. Do NOT just increase BLOCK_SIZE to reduce atomic count — that is a micro-optimization that leaves the structural bottleneck in place.
- When auxiliary-op fusion has been attempted and blocked by correctness, precision, or compile constraints (e.g., fp16 Inf mismatch, dtype associativity), and the torch wrapper chain still dominates `total_op_avg_time_us` with multiple aclnn ops each allocating a temporary tensor, fall back to wrapper-level buffer reuse. Read [`../triton-npu-optimize-knowledge/references/patterns/wrapper-buffer-reuse.md`](../triton-npu-optimize-knowledge/references/patterns/wrapper-buffer-reuse.md) and apply pre-allocated buffer reuse via the `out=` parameter, in-place `+=` accumulation, and an in-place pass-through kernel to eliminate the output copy. This is a fallback direction — do not choose it before attempting fusion.
