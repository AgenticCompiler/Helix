...
Two independent phases. Either or both may apply depending on the kernel's current state.
...
1. **Swap loop nest to eliminate redundant loads** (highest priority, often 10-30× speedup alone)
   ...
   - **When a fused downstream operation groups outputs** (e.g., maxpool windows, reduce over output chunks), apply loop swap and/or widen the output tile to cover multiple downstream groups per tile — both reduce redundant x loads, and are complementary when combined. Each step also works independently — apply whichever is feasible.

   ```python
   # detect: x loaded inside the window loop — redundant per window
   for win in range(0, OUT_F // KERNEL):          # KERNEL = window size (e.g., 2)
       for in_tile in range(0, IN_F, BLOCK_IN):
           x_vec = tl.load(x_ptr + in_offs, ...)  # reloaded every window
           w_tile = tl.load(w_ptr + win * KERNEL + ..., ...)
           acc += tl.sum(w_tile * x_vec[None, :], axis=1)
           # ... fused maxpool on this window's outputs ...
   ```

   ```python
   # restructure: widen output tile to cover multiple windows, x loaded once per in_tile per output tile
   WINDOWS_PER_TILE = BLOCK_O // KERNEL           # e.g., BLOCK_O=64, KERNEL=2 → 32 windows
   for win_group in range(0, OUT_F // KERNEL, WINDOWS_PER_TILE):
       acc = tl.zeros([BLOCK_O], dtype=tl.float32)
       for in_tile in range(0, IN_F, BLOCK_IN):
           x_vec = tl.load(x_ptr + in_offs, ...)  # loaded once per in_tile within this output tile
           w_tile = tl.load(w_ptr + win_group * KERNEL + ..., ...)  # [BLOCK_O, BLOCK_IN]
           acc += tl.sum(w_tile * x_vec[None, :], axis=1)
       # ... fused maxpool on BLOCK_O outputs, extract per-window maxima via gather/mask ...
   ```
