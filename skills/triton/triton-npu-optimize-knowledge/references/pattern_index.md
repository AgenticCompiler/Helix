...
- Use When:
  ...
  - **Pre-Gate (always check first):** A-Cat-5 fires when `[MTE2 Data Transport]` ProcessBytes avg < 128B or Data movers = 0 with `tl.load` present. Firing is a hint, not a final ruling — verify the signal is not a false positive (check kernel body for pure data-movement, check `[SCALAR Instr Types]` for real address-computation dominance, check host-side strides). If verified as a true positive, stop — autotune cannot fix fragmented layout; fix memory layout/compiler hints first. If verified as a false positive, proceed to Phase 2.
  - **Parameter Diagnostics (only if Pre-Gate passed or A-Cat-5 verified as false positive):** A-Cat-6 (Register Spilling), A-Cat-1 (Pipeline Overlap Deficit), A-Cat-2 (Scalar Overhead Dominance) are diagnosed from `report.txt`. A-Cat-3 (Parallelism Starvation) and A-Cat-4 (Autotune Key Mismatch) require additional data sources.
