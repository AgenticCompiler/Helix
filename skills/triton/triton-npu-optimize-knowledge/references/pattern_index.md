...
- Summary: Move NPU work that the NPU does **poorly or redundantly** to CPU (numpy/scipy), then bring results back with `torch.from_numpy()`. A **wrapper-level structural pattern**: it changes *where* work runs without rewriting the Triton kernel; evaluate it before kernel-level micro-optimizations. Three flavors share one precondition (moderate data volume, bulk D2H/H2D amortized, CPU not saturated):
  - **Flavor A — one-shot scalar/integer wrapper ops**: `torch.unique`, `torch.sort`/`argsort`, `torch.searchsorted`, `torch.nonzero`, `torch.bincount`. The NPU scalar pipeline is the bottleneck; numpy's C path wins on small-to-medium integer tensors.
  - **Flavor B — per-iteration compute inside a sequential launch loop** (higher-impact, easily missed): a Python `for`/`while` loop that **each iteration** launches an NPU kernel (Triton or aclnn) over a per-iteration tail *and* reads back per-iteration state (`.item()`/`.cpu()`/`.tolist()`) to pick the next item. Launch×count + sync×count dominates; the per-iteration compute is vectorizable over the tail, so a numpy expression can replace the kernel body losslessly. **Often the NPU kernel's output turns out redundant (never consumed / recomputed on CPU) — deleting it is pure win, not a trade-off.**
  - **Flavor C — one-shot small-data wrapper-helper pipeline** (highest-frequency in practice, most easily missed): a helper function (not the Triton kernel) builds small intermediate tensors (indices / weights / pos_ids / freqs / cos·sin / permutation index) through a chain of general-purpose NPU ops — `torch.arange`/`linspace`/`mul`/`add`/`clamp`/`stack`/`cat`/`cos`/`sin`/`outer`/fancy-index/`repeat`. No single op dominates but their **combined** launch overhead does; each op's data is only a few KB–hundreds of KB. Rewriting the *entire helper* in numpy + one `torch.from_numpy().to(device)` replaces N aclnn launches with one H2D copy. In observed cases this is the difference between stopping at ~2.6x ("the remaining ops are essential") and breaking through to ~30x.
- Source: [cpu-offload-dedup.md](patterns/cpu-offload-dedup.md)
- Use When (Flavor A — one-shot scalar wrapper ops):
  - Wrapper code contains NPU ops that are **inherently scalar/integer-based**: `torch.unique`, `torch.searchsorted`, `torch.nonzero`, `torch.bincount`, `torch.sort`, `torch.argsort`, or equivalent.
  - The input data for these ops is small-to-medium integer tensors (typically ≤1M elements of int32/int64).
  - Profiling shows these NPU ops have **disproportionate latency** relative to their data volume (e.g., 30-80us for sorting/finding a few hundred integer IDs).
  - The results (unique IDs, inverse indices, sorted order) are needed **before** the main Triton kernel for reduced computation (e.g., dedup to compute group-by aggregation on fewer rows).
- Use When (Flavor B — sequential launch loop) — **if any of these appear, open the full pattern file before dismissing this pattern**:
  - A Python `for`/`while` loop iterates over selected items (hundreds to a few thousand iterations) and, **each iteration**, launches an NPU kernel (Triton or aclnn) over a per-iteration tail *and* reads back per-iteration state via `.item()`/`.cpu()`/`.tolist()` to pick the next item.
  - The per-iteration work is **embarrassingly vectorizable over the tail** (each candidate independent) — so the kernel body has a 1:1 numpy equivalent (elementwise + `np.where` + indexed write).
  - Profiling shows the per-iteration NPU kernel is invoked **thousands of times** and dominates NPU time, while each invocation's useful compute is small (launch/DMA-setup-bound, not compute-bound).
  ...
  - **Do NOT dismiss Flavor B by reasoning "the per-iteration compute must stay on NPU."** First run the file's Step 1 diagnostic: trace whether the per-iteration kernel's output tensor is ever consumed. If it is never read back (the loop reads a CPU mirror, or recomputes the same quantity on CPU), the NPU kernel is **redundant work** → delete it (pure win). Only if the output is read back *every iteration* do you offload state + compute **together** (never state alone — see the file's Cross-device RMW trap). The "IoU/inner compute must stay on NPU" intuition is exactly the trap this diagnostic is designed to overturn.
  - **Do NOT reject Flavor B as a "pure-PyTorch rewrite / bypassing the Triton NPU path."** That rule forbids reimplementing the operator's *bulk-parallel* compute on host; Flavor B only relocates the irreducibly-sequential per-iteration loop (or deletes redundant NPU work) and **keeps at least one bulk-parallel Triton kernel on the NPU path**. It is a wrapper-level / data-movement change, explicitly permitted, and a Flavor B round **does count as a successful optimize round**. See the file's **Scope Boundary** section for the exact boundary rule before rejecting on this ground.
- Use When (Flavor C — one-shot small-data wrapper-helper pipeline) — **if any of these appear, open the full pattern file before dismissing this pattern; this flavor is the one most often missed because the ops look "essential"**:
  - A wrapper helper function (named `_build_*` / `_precompute_*` / `_make_*`) — **not** the Triton kernel — builds small intermediates (indices, weights, pos_ids, freqs, cos/sin, permutation index) via a chain of general-purpose NPU ops (`torch.arange`/`linspace`/`mul`/`add`/`clamp`/`stack`/`cat`/`cos`/`sin`/`outer`/fancy-index/`repeat`) that feed the Triton kernel.
  - Each op's data is small (a few KB to a few hundred KB); the helper's total output is well under 1MB.
  - Profiling shows **many distinct `aclnn*` ops** (`aclnnArange`/`Linspace`/`Mul`/`Add`/`Clamp`/`Stack`/`Cat`/`Cos`/`Sin`/`Index`/`Repeat`/`InplaceCopy`) each costing a few µs–tens of µs; **no single op dominates but their sum is >50% of total-op** — this "many small ops" profile is the signature.
  - The Triton gather/reduction kernel itself is already cheap (typically <15% of total-op) — the bottleneck is wrapper-op launch overhead, not kernel compute. (If the kernel is the bottleneck, this flavor does not apply.)
  - A `for` loop over grid/batch entries calls `.item()` per iteration to read NPU scalars, triggering a device→host sync each time.
  - **Do NOT dismiss Flavor C by reasoning "cos/sin/freqs/arange are core compute and must stay on NPU."** The no-rewrite rule protects the **bulk-parallel Triton kernel doing genuinely parallel work over large data** — it does *not* protect small-data elementwise/construction wrapper ops that happen to produce the kernel's inputs. `torch.cos`/`torch.sin`/`torch.arange`/`torch.cat` on a few-KB tensor are wrapper ops, not core compute. "Core compute" = bulk-parallel work over large data where NPU data locality wins; classifying the former as the latter and stopping is the exact failure mode that leaves 15–30x of speedup on the table. See the file's **"core compute" clarification** under Scope Boundary.
  - **Do NOT reject Flavor C as a "pure-PyTorch rewrite."** Same boundary rule as Flavor B: keep at least one bulk-parallel Triton kernel on the NPU path (the gather/reduction that consumes the helper's output); only the small-data wrapper helper moves to CPU. A Flavor C round **does count as a successful optimize round**.
