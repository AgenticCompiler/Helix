# Algebraic optimization (pure math reformulation)

## Summary

Apply mathematical identities and semantics-preserving rewrites to reduce redundant memory passes, full data scans, or live ranges before micro-tuning loads. Covers floating-point identities (e.g., single-pass mean/variance), operator-defined equivalences (e.g., PyTorch logical ops under dtype-specific truthiness and broadcasting), and algebraic merge rules.

## Use When

- The hot path performs **two or more full traversals** of the same data for statistics, normalization, or mergeable closed-form subexpressions.
- Profiler or IR suggests **duplicate MTE-heavy** phases that differ only by a scalar statistic of the same tensor.
- Elementwise **logical** ops (`logical_or`, `logical_and`, …) use **broadcasting**, and truth tests (`ne`, `!= 0`) run on **fully expanded** numeric tensors.
- Pairwise gated tiles compute `exp(g_i - g_j)` only as a multiplicative factor and can use row/column broadcast factors instead.
- You want fewer global passes or cheaper elementwise work **before** changing tile sizes, pipelines, or autotune grids.
- A scalar expression `a * b + c` feeds into a math intrinsic (`rsqrt`, `exp`) and profiling shows scalar instruction pressure. Use `tl.math.fma` to fuse multiply-add into one hardware instruction.

## Avoid When

- The bottleneck is clearly **only** bad tile size or UB overflow with **no** redundant algorithmic passes (prefer `tiling` or footprint patterns first).
- Custom Triton fusion is attempted before a simpler **host/graph reorder** is proven correct and cheaper end-to-end.

## Signals

### Code

- Two loops or kernels with nearly identical `tl.load(x)` tiling along the same axis.
- `broadcast_tensors(x, y)` followed by elementwise truth tests on **wide dtypes** over the full broadcast shape.

### Profile

- `NotEqual` / `BroadcastTo` (or equivalent ops) scale with **broadcast-expanded** `numel`, not with `numel(x) + numel(y)`.

### IR

- Repeated load or reduction structure around the same logical axis where a single pass could feed multiple accumulators (case-dependent).

## Related Patterns

- `tiling`
- `program-multiple-rows`
- `software-pipeline`
- `slice-intermediate`
- `autotune`
- `compiler-intrinsic-lowering-analysis` — intrinsic-level substitution after math structure is stable
- `eliminate-intermediate-precision-cast` — bounding-transformation simplification
- `route-reduction-through-matrix-unit` — specialized case that replaces mathematically equivalent expressions with faster hardware routing
- `consolidate-narrow-loads-into-2d` — host-side coefficient pre-combination is a form of algebraic pre-fusion
- `intra-kernel-pass-fusion` — eliminates memory round-trips while keeping the same math structure

## What To Verify After Applying

- Correctness vs reference: **FP order**, **NaN**, **dtype promotion**; for logical ops also **bool**, **complex**, **empty tensors**, and broadcast corners.
- Profiler: fewer passes or lower time on the merged phase for the **same** outputs.
- UB and launch: the rewrite does not force unsafe per-program working sets on NPU.

---

## Feature scan checklist

Use this as a **grep-for-the-brain** over source and profiler hints. If **yes**, consider an algebraic rewrite *before* micro-tuning loads.

| # | Feature in code / profiler | Typical algebraic lever |
|---|----------------------------|-------------------------|
| F1 | **Repeated full scans** of the same tensor axis for statistics (e.g. `sum` then second pass with `mean`) | Fuse passes via **extra accumulators** per tile (moments, prefix sums, running aggregates) |
| F2 | **Redundant recomputation** of the same closed-form subexpression inside a loop | **Hoist** or replace with **incremental / merged** formula |
| F3 | **Explicit normalization** `x / sqrt(eps + sum(...))` with a reduction that could share partials | Combine **variance and norm** derivations so one reduction feeds both |
| F4 | **Block-wise** algorithms that **re-load** the same block to apply a global statistic | Merge blocks using **algebraic merge rules** (e.g. parallel prefix, Welford merge) so **one load** feeds merge |
| F5 | **Symmetric** or **idempotent** math that suggests halving work (e.g. `min(a,b)+max(a,b)`) | Exploit identities to **cut evaluations** |
| F6 | **Elementwise logical** after **broadcast** where truth tests run on **fully expanded** numeric tensors | **Truth map on original shapes**, then **broadcast bool masks**; **`bool` identity** short-circuit; **empty** `out_shape` early exit |
| F7 | **Pairwise gated tiles** use `exp(g_i - g_j)` only as a multiplicative factor | Factor into row/column vector terms such as `exp(g_i) * exp(-g_j)` to avoid materializing the pairwise difference |

**Profiler corroboration (Ascend / msprof-style):** duplicate hot `tl.load` regions, high `MOV_*` / `WAIT_FLAG` in phases that differ only by a scalar derived from the same data—the workload may be **memory- or sync-bound** and a good candidate for **fewer passes**.

---

## Principles

1. **Prove equivalence** under the operator’s reference semantics (real-number model where relevant; then **NaN**, **dtype promotion**, **complex**, **logical** truthiness as needed).
2. **Measure**: some equivalent forms have **longer dependency chains** or **higher UB peak**; algebraic “fewer ops on paper” can **regress** after lowering.
3. **Document** new catalog entries with: *when*, *signals*, *rewrite*, *why it can help on NPU*, *risks*, *verification*.

---

## Technique catalog

Each subsection is an independent pattern. Add new `### Case N` entries over time.

### Case 1: Single-pass first and second moments (mean & variance)

**When to use**

- You need **mean** and **(population) variance** (or sum of squared deviations) along a reduction axis.
- The baseline uses **two full traversals** of that axis: e.g. `sum(x)` then `sum((x - mean)**2)` or separate kernels for mean and variance.

**Signals**

- Two loops (or kernels) with **nearly identical** `tl.load(x)` tiling along the same axis.
- OPPROF: statistics phase dominated by **MTE** / **wait-on-flag**, not by vector FLOPs alone.
- Common in **LayerNorm**, **InstanceNorm**, **batch variance** along a contiguous dimension.

**Rewrite (algebra)**

For length \(n\), \(\mu = \frac{1}{n}\sum_i x_i\), population variance \(\sigma^2 = \frac{1}{n}\sum_i (x_i-\mu)^2\).

Single traversal:

- \(S_1 = \sum_i x_i\), \(S_2 = \sum_i x_i^2\)
- \(\mu = S_1/n\), \(\sigma^2 = \max(0,\, S_2/n - \mu^2)\) (clamp fixes tiny negative FP noise before `rsqrt`)

**Why it can improve performance on NPU**

- **One fewer full memory pass** over the axis for statistics; MTE and loop synchronization are often amortized.
- One `tl.load(x)` can feed **two reductions** (`sum(x)`, `sum(x*x)`).

**Risks**

- **Numerical**: \(S_2/n - \mu^2\) can be less stable than two-pass centered sums for extreme magnitudes; align with product numerics requirements.
- **UB / registers**: holding both accumulators plus `x*x` may **raise peak UB** vs. one accumulator; co-tune with **`tiling`** / **`program-multiple-rows`**.
- **Alternatives**: **block-wise Welford merge** keeps a single pass but adds **merge arithmetic**; on some stacks it **does not** beat `S1+S2`—**benchmark**.

**Verification**

- Compare against reference (e.g. `torch.nn.functional.layer_norm`) on representative dtypes/shapes.
- Confirm profiler: **fewer** attributed loads or cycles in the statistics phase vs. two-pass baseline.

---

### Case 2: Truth map before broadcast for elementwise logical ops (`logical_or` / friends)

**Classification (is this “pure math”?)**

- **Yes, in the sense of this catalog**: it is a **semantics-preserving rewrite** under the **reference operator definition** (e.g. `torch.logical_or`), not a change to the intended output values.
- It is **not** a continuous real-number identity like Case 1; it is **logical / discrete** equivalence plus **broadcast algebra**: truthiness commutes with broadcasting so that per-output-index results match `logical_or(broadcast(x), broadcast(y))`.
- **Prove equivalence** under the reference (including `NaN`, `complex`, `bool`), then **measure**—fusion into custom Triton is optional and may still regress on a given NPU stack.

**When to use**

- Implementing **`torch.logical_or(x, y)`** (or similar) with **broadcasting**, where the naive pipeline does: `xb, yb = broadcast_tensors(x, y)` then **`ne` / truth tests on the full expanded numeric tensors**.
- You want fewer elementwise comparisons (and often less pressure from large temporary/broadcast paths) **before** custom Triton or mask kernels.

**Signals**

- Profiler / op statistics: **`NotEqual`** (or `Cast` around masks) scales with **broadcast-expanded** `numel`, not with `numel(x)+numel(y)`.
- Heavy **`BroadcastTo`** (or equivalent) appears **before** truth tests on wide dtypes (`fp16`/`bf16`/`fp32`).
- Shapes like `(N,1)` vs `(1,N)` or `(N,…,K)` vs `(N,1,…,1,K)` where expanded compares would touch **Θ(product of output dims)** on both operands.

**Rewrite (equivalence sketch)**

Let `T` be the reference **truth map** for one tensor (PyTorch: `logical_or` uses truthiness; for non-complex numerics, `T(a) = (a != 0)` matches `torch.ne` for the usual cases; `bool` is already boolean).

For broadcast output index `i` with preimage indices `i_x` into `x` and `i_y` into `y` (standard broadcast rules):

\[
\text{logical\_or}(x,y)[i] = T(x[i_x]) \lor T(y[i_y]) = \big(T(x) \text{ broadcast} \lor T(y) \text{ broadcast}\big)[i].
\]

Equivalent pipeline:

1. **`tx = T(x)`, `ty = T(y)`** on **original** shapes.
2. **`txb, tyb = broadcast_tensors(tx, ty)`** as **bool** (or fuse `∨` without materializing full numeric broadcast).
3. Micro-optimizations:
   - **`bool` short-circuit**: if `x.dtype is bool`, then `tx = x` (no `ne`).
   - **Empty output**: if `prod(out_shape)==0`, return `empty(out_shape, bool)` without broadcasting inputs (use `broadcast_shapes` for metadata).

**Why it can improve performance on NPU**

- **Fewer elementwise truth tests** on **expanded** floating-point buffers: compare each **stored** element of `x` and `y` once, then broadcast **bool** (narrower).
- Profiler: can shrink **`NotEqual`** / **`BroadcastTo`** when those were tied to naive ordering.

**Risks**

- **Semantics**: match reference for **`complex`** (`view_as_real` + combine real/imag), **`NaN`**, **`bool`**, and dtype rules—do not assume `!= 0` without checking each dtype.
- **Fusion is separate**: lowering to Triton may **regress** (indexing, UB, launch)—validate independently.

**Verification**

- Compare vs **`torch.logical_or(x, y)`** on dtypes/shapes (broadcast corners, empty, `NaN`, `complex` if supported).
- Profiler: reduced **`NotEqual`** / **`BroadcastTo`** before optional custom kernels.

---

### Case 3: Pairwise gated exponential factorization

**When to use**

- A causal or pairwise tile multiplies by a gate term like `exp(g_i - g_j)`.
- `g_i` and `g_j` come from the same one-dimensional time vector inside a chunk.
- The pairwise difference matrix is only used as input to `exp` and then multiplied into another tile.
- Broadcasted row/column factors can reduce live intermediates or dependency depth.

**Signals**

- Code builds `b_g_diff = b_g[:, None] - b_g[None, :]`.
- The next operation is only `b_A *= exp(b_g_diff)` or equivalent.
- The same tile already has a row-side factor, such as `beta[:, None]`, that can be merged with `exp(g_i)[:, None]`.

**Rewrite (algebra)**

For every pair `(i, j)`:

\[
\exp(g_i - g_j) = \exp(g_i) \cdot \exp(-g_j)
\]

Triton-style rewrite:

```python
# Before
b_g_diff = b_g[:, None] - b_g[None, :]
b_A *= exp(b_g_diff)

# After
b_A *= exp(b_g)[:, None] * exp(-b_g)[None, :]
```

If a row-side scale already exists, fold it into the row factor:

```python
b_A *= (b_beta * exp(b_g))[:, None] * exp(-b_g)[None, :]
```

**Why it can improve performance on NPU**

- Avoids an explicit `[BT, BT]` pairwise difference intermediate.
- Lets the compiler represent the gate as broadcasts from two `[BT]` vectors.
- Can shorten the vector dependency chain around a `tl.dot`-produced attention or recurrence tile.

**Risks**

- The factored form may evaluate two vector exponentials instead of one matrix-shaped expression; benchmark the actual lowering.
- Large positive/negative `g` values may expose different overflow/underflow timing even when the real-number identity is exact.
- If `g_i - g_j` is reused by multiple downstream expressions, keeping the explicit difference can be cheaper.
- Check NaN/inf propagation if the operator has strict edge-case semantics.

**Verification**

- Compare numerical outputs on representative gate ranges, including large magnitude values.
- Confirm profiler/IR no longer shows the targeted pairwise diff materialization or that runtime improves despite equivalent IR.
- Re-profile after adjacent layout or mask changes; this rewrite often interacts with tile live range.

---

### Case 4: 2-class softmax to sigmoid replacement

**When to use**

- A kernel computes a 2-element softmax over two scores `[a, b]` whose output is a weighted linear combination `softmax([a,b])[0] * X + softmax([a,b])[1] * Y`.
- The kernel reduces two related dot products into these two scores and uses them only for this softmax and weighted combination.

**Signals**

- Two dot products feed a `softmax(dim=-1)` with exactly 2 elements, or an explicit `exp(a)`, `exp(b)`, `max`, and division.
- The softmax weights are consumed only by linear combination with two value tensors.
- Profiling shows the softmax or exp region as a compute bubble with serialized vector instructions.

**Rewrite (algebra)**

`softmax([a,b])[0] = sigmoid(a-b) = 1/(1+exp(b-a))`.

Two algebraically equivalent forms for the weighted combination:

```python
# Form A (saves one multiply: 1 FMA + 1 add):
diff = score_ul - score_ur  # or: sum(Q * (K_ul - K_ur)) * scale
attn = 1.0 / (1.0 + tl.math.exp(-diff))
o = V_ur + attn[:, None] * (V_ul - V_ur)

# Form B (explicit 2-weight form):
attn = 1.0 / (1.0 + tl.math.exp(-diff))
o = attn[:, None] * V_ul + (1.0 - attn[:, None]) * V_ur
```

Form A is preferred when combined with loop-invariant hoisting: precompute `V_ul - V_ur` and `K_ul - K_ur` once outside any inner group loop.

**Why it can improve performance on NPU**

- Cuts transcendental ops from 2 exp + max + div to 1 exp + reciprocal. On NPU, exp may be a serializing instruction; removing one exp reduces compute bubbles.
- Combined with V diff precomputation: replaces 2 exp + 2 mul + 1 add + 1 sub + 1 max with 1 exp + 1 reciprocal + 1 mul + 1 add per output position.

**Risks**

- `sigmoid(a-b)` is equivalent to `softmax([a,b])[0]` in exact arithmetic but may produce different roundoff. For large |a-b|, one path may overflow where the other underflows.
- Only applies to exact 2-class softmax. Do not apply to softmax with >2 elements.
- If scores feed other consumers (e.g., log-sum-exp tracking), the simplification breaks those uses.

**Verification**

- Compare outputs against the 2-class softmax reference on representative score ranges, including extremes.
- Confirm profiler shows reduced vector instruction count in the score-to-attention region.
- Verify both MHA and GQA paths, and both float32 and bf16 outputs.

---

### Case 5: Modulo-to-multiply-subtract in coordinate/index decomposition

**When to use**

- A kernel computes both `a // b` (integer division) and `a % b` (modulo) on the same operands in a coordinate/index decomposition chain.
- The target NPU lacks hardware vector integer modulo, so `%` lowers to an expensive scalar software routine per lane.
- The integer division quotient is already computed and needed for the next decode level — it is live and reusable.

**Signals**

- Code: `coord = linear // stride; residual = linear % stride;` or similar chains in flat index decode or coordinate recovery.
- Profiling: high `aiv_scalar_ratio` driven by per-element integer modulo operations.
- IR: scalar modulo or divide-remainder lowering on a vector path.

**Rewrite (algebra)**

Identity: `a % b = a - (a // b) * b` for non-negative `a` and positive `b`.

```python
# Before: 1 div + 1 mod per coordinate level
coord2 = residual // out_stride2
coord3 = residual % out_stride2

# After: 1 div + 1 mul + 1 sub per coordinate level
coord2 = residual // out_stride2
coord3 = residual - coord2 * out_stride2
```

**Why it can improve performance on NPU**

On NPUs without vector integer modulo hardware, `%` scalarizes into a software routine per element. Multiply-subtract reuses the quotient already computed by `//`, replacing an expensive scalar modulo with one vector multiply and one vector subtract. For a 3-level decode chain, this saves 3 scalar modulo ops per element.

**Risks**

- Only applicable when both `a // b` and `a % b` appear together. A standalone `%` without `//` cannot use this identity.
- The identity holds for non-negative `a` and positive `b` — always true for size- and stride-based index arithmetic.
- For floating-point operands this identity does not hold; only apply to integer-valued index/offset arithmetic.

**Verification**

- Compare output against the modulo-based reference for all shapes and boundary indices.
- Verify the rewritten expression produces the same coordinate mapping.
- Confirm profiler shows reduced scalar time and fewer scalar modulo traces.

---

### Case 6: Sorted-invariant reduction to scalar access

**When to use**

- A reduction (`max`, `min`, `argmax`, `argmin`) runs over a 1D vector known to be sorted along the reduction axis.
- The sorted order is guaranteed by a preceding operation (topk, argsort, sort) or by data construction.

**Signals**

- `tl.max(x, axis=0)` or `tl.min(x, axis=0)` on a vector right after data is sorted.
- Profiling shows the reduction as scalar-heavy time on Ascend NPU.
- The sort is visible in the same kernel or in host-side preprocessing.

**Rewrite (algebra)**

Descending-sorted: `tl.max(x, axis=0)` → `tl.load(x_ptr)` (scalar load of first element).
Ascending-sorted: `tl.min(x, axis=0)` → `tl.load(x_ptr)`.
For `argmax` on descending-sorted data, the result is 0.

```python
# Before: full-vector reduction over N elements
logits_max = tl.max(logits, 0)

# After: scalar load of first element (sorted descending)
logits_max = tl.load(row_logits_ptr)
```

**Why it can improve performance on NPU**

Replaces a full-vector reduction with a single scalar load. On Ascend NPU, 1D `tl.max` lowers to scalar accumulation per lane; a scalar load is strictly cheaper.

**Risks**

- The sorted invariant must be provable and not break across dtypes or code paths.
- If data is only partially sorted (e.g. per-block), this does not apply.
- Changing the upstream operation to one that does not sort silently breaks correctness.

**Verification**

- Prove the sorting invariant from upstream operations or data construction.
- Compare outputs against reference for all input shapes and dtypes.
- Confirm profiler shows the reduction time eliminated.

---

### Case 7: Cumulative-proportion filtering without full normalization

**When to use**

- A pipeline computes `softmax → cumsum → threshold → mask → re-softmax` on sorted values.
- The intermediate normalization (division by sum) feeds only the threshold comparison and final probabilities.
- One or more cumsums can be reused instead of recomputed after masking.

**Signals**

- Two exp or two softmax regions on the same data, separated by a threshold mask.
- Code pattern: `probs = x / sum(x)` → `cumsum(probs)` → `mask = (cumsum - probs) > t` → `filter(x)` → `exp(x - max)` → `x2 / sum(x2)`.
- Profiling shows `tl.cumsum` as dominant scalar time (high aiv_scalar_ratio).
- A second cumsum appears after masking that could reuse an earlier cumsum.

**Rewrite (algebra)**

The filter condition `cumsum(probs) - prob > t` is equivalent to `cumsum(n) - n > t * sum(n)` where `n = exp(logits - max)`. This eliminates per-element division by `sum(n)`.

After masking with `where(mask, 0, n)`, the kept cumulative sum equals the pre-existing cumsum for all pre-cutoff positions — no second cumsum is needed. Threshold comparison reuses the original cumsum directly.

```python
# Before: normalize probs → cumsum → filter → re-exp → re-normalize → cumsum again
probs = numerator / tl.sum(numerator, 0)
cum_probs = tl.cumsum(probs, 0)
to_remove = (cum_probs - probs) > top_p
filtered_logits = tl.where(to_remove, -inf, logits)
f_numerator = tl.exp(filtered_logits - tl.max(filtered_logits, 0))
f_probs = f_numerator / tl.sum(f_numerator, 0)
f_cum_probs = tl.cumsum(f_probs, 0)

# After: cumsum raw numerators → scale threshold → mask numerators → reuse cumsum
cum_numerators = tl.cumsum(numerator, 0)
to_remove = (cum_numerators - numerator) > (top_p * tl.sum(numerator, 0))
kept_numerator = tl.where(to_remove, 0.0, numerator)
# Reuse cum_numerators for downstream threshold comparison
is_candidate = cum_numerators >= (threshold * tl.sum(kept_numerator, 0))
```

**Why it can improve performance on NPU**

Eliminates one division pass (N elements), one second exp pass, one second sum reduction, and one `tl.cumsum` — the dominant scalar-bound operation on Ascend NPU.

**Risks**

- Equivalence depends on `sum(n) > 0`, which holds when at least one logit is finite.
- Working with raw numerators can produce larger intermediate magnitudes than normalized probabilities, though subtract-max bounds this.
- If the sort order or cumsum monotonicity is not guaranteed, the cumsum reuse breaks.

**Verification**

- Compare outputs against the reference pipeline for all shapes and dtypes.
- Verify the second exp/sum/cumsum region is absent from the IR.
- Confirm the filter mask produces correct results at boundary cases (all kept, all removed, single token).

---

### Case 8: Host-precomputed scalar inverses (division-to-multiplication)

**When to use**

- The kernel divides by a scalar known before launch (hyperparameter, shape constant), not per-element data.
- The NPU target has expensive vector division relative to multiplication.

**Signals**

- Kernel code divides by a scalar computed from kernel arguments: `x / (1.0 - beta_power)`.
- Profiler: vector division on an otherwise compute-light path.

**Rewrite (algebra)**

Identity: `x / s = x * (1.0 / s)`.

Precompute the inverse on the host and pass it as a kernel argument:

```python
# Host side
inv_b1c = 1.0 / (1.0 - float(beta1_power))
inv_b2c = 1.0 / (1.0 - float(beta2_power))

# Kernel: multiplication replaces division
m_hat = m_new * inv_b1c
v_hat = v_new * inv_b2c
```

**Why it can improve performance on NPU**

Division is 4-8x more expensive than multiplication on Ascend NPU vector units. The host precomputation costs one scalar operation. Removing division frees vector cycles for better compute-memory overlap, even on memory-bound kernels.

**Risks**

- `x * (1.0/s)` may differ from `x / s` in the LSB due to rounding. Verify with workload-appropriate tolerance.
- Each precomputed inverse adds one kernel argument.
- Only applies to uniform scalar divisors. Per-element divisors cannot use this pattern.

**Verification**

- Compare against division-based reference on representative shapes and dtypes.
- Confirm kernel no longer contains division instructions for the replaced scalars.

---

### Case 9: Fused multiply-add via tl.math.fma

**When:** Expression `a * b + c` feeds into a math intrinsic (`rsqrt`, `exp`) on hardware with FMA support.

**Rewrite:** `rsqrt(tl.math.fma(s, inv_n, eps))` replaces `rsqrt(s * inv_n + eps)`.
`tl.math.fma` lowers to one FMA instruction instead of separate multiply and add.

**Why:** Saves one scalar instruction from the reduction-broadcast pipeline. Removes scalar pipe pressure when the reduction feeds into `rsqrt` or similar intrinsics.

**Risks:** FMA rounds once (fused); separate mul+add rounds twice. Negligible difference for float32 accumulators.

**Verify:** Compare against separate multiply-add reference on representative shapes and dtypes.

---

## Relation to other patterns

Apply `tiling`, `program-multiple-rows`, `software-pipeline`, `slice-intermediate`, or `autotune` after the math structure is stable. Prefer one algebraic change per round, then re-profile. Use Cases 1-9 as indexed by their signals.
