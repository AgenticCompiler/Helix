---
name: triton-npu-optimize-knowledge-distill
description: Generic reference-only optimize knowledge (distill working copy) for pattern triage and evidence-backed symptom routing. This skill does not define optimize workflow or own round artifacts; prefer this tree for distillation-driven knowledge updates.
---

# Optimize Knowledge (distill)

## Purpose

This skill is the generic optimize knowledge library for reusable pattern and symptom references. It is a **copy of `triton-npu-optimize-knowledge`** intended as the **distill update target**; keep `triton-npu-optimize-knowledge` unchanged unless you are intentionally aligning the bases.
...
- For evidence-to-pattern distillation, follow the `ascend-npu-distill-patterns` skill rather than inventing a local process.
