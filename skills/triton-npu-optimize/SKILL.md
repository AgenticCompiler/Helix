...
- **Autotune-signal as mandatory candidate:** When hardware profiling data (`sim_features.txt`, msprof execution trace logs, or profiler `op_summary_*.csv`) exists in the workspace or current `opt-round-N/` directory, you **must** read [`../triton-npu-optimize-knowledge/references/patterns/autotune-signal.md`](../triton-npu-optimize-knowledge/references/patterns/autotune-signal.md) and execute the signal matching flow (check each Category in priority order: Cat 5 → Cat 6 → Cat 1 → Cat 2 → Cat 3 → Cat 4). If any Category fires, `autotune-signal (Cat N)` **must** appear as a candidate pattern in the `attempts.md` candidate list. When the fired signal involves autotune parameter search (`BLOCK_*`, `num_stages`, `num_warps`, or `key` tuning), route to the `autotune` pattern and construct candidate `triton.Config` lists following the fired Category's Optimization Direction. Record the signal matching result and reasoning explicitly in `attempts.md`.
