...
- **Autotune-signal as mandatory two-phase check:** When `extracted_bin_data/report.txt` exists in the workspace or current `opt-round-N/` directory, you **must** read [`../triton-npu-optimize-knowledge/references/patterns/autotune-signal.md`](../triton-npu-optimize-knowledge/references/patterns/autotune-signal.md) and execute both phases. Execute this check **after** the simulation-signal check (scalar-vector-simulation-signal) so that code-level signals are evaluated first. **Phase 1 (Pre-Gate):** Check A-Cat-5. If A-Cat-5 fires, autotune is not suitable — route to `compile_hint` or `discrete_memory_access` to fix memory layout/compiler information, and exit the autotune flow. Do not proceed to Phase 2 until the memory layout issue is resolved and a new simulation confirms the Pre-Gate passes. **Phase 2 (Parameter Diagnostics):** Only if the Pre-Gate passes — check A-Cat-6 → A-Cat-1 → A-Cat-2 → A-Cat-3 → A-Cat-4 in order. If any Category fires, `autotune-signal (A-Cat-N)` **must** appear as a candidate pattern in the `attempts.md` candidate list. When the fired signal involves autotune parameter search (`BLOCK_*`, `num_stages`, `num_warps`, or `key` tuning), route to the `autotune` pattern and construct candidate `triton.Config` lists following the fired Category's Optimization Direction. For A-Cat-3 and A-Cat-4, which require data beyond `report.txt` (hardware profiling and multi-shape benchmark respectively), note the data gap explicitly and treat the signal match as a hypothesis to validate rather than a confirmed trigger. Record the signal matching result and reasoning explicitly in `attempts.md`.
