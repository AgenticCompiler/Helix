...
- Use the sibling `npu-optimize-submit-round` skill to submit the current round (with `--min-rounds <N>` when the session has a minimum round requirement) and repair the round until it passes before continuing or stopping.
