...
- **Pattern routing via `pattern_index.md`:** Read [`../triton-npu-optimize-knowledge/references/pattern_index.md`](../triton-npu-optimize-knowledge/references/pattern_index.md) as the primary routing entry point. It contains a `## Signal-Based Pattern Routing` section with three ordered steps. Follow them in order; the first matching step wins:
  - **Step 1 — Code Structure Priority:** Always execute. Check code features against the table. Each row lists a code feature, candidate patterns, and differentiation criteria. This step requires no profiling data. If a code feature matches, record the candidate pattern(s) and proceed to read the corresponding detailed pattern card(s).
  - **Step 2 — Simulation Signal Routing (`extracted_bin_data/report.txt`):** Execute **only** when `extracted_bin_data/report.txt` exists and has been read. This file is present only in simulation mode. When `report.txt` is available, scan its sections in the order specified by the routing table (`[Pipeline Flows]` → `[MTE2 Data Transport]` → `[Pipe Distribution]` → `[Pipe Overlap Ratio]` → `[VECTOR Unit]` → `[TRACE Events]`). For each section, check feature conditions against the report data. Each matching row gives candidate patterns and differentiation criteria (typically code features). Record which sections matched, which feature conditions fired, and the resulting candidate patterns in `attempts.md`. When `report.txt` is absent, skip this step entirely and continue to Step 3 or fall back to the normal `## High Priority Patterns` and `## Generated Pattern Summaries` sections in `pattern_index.md`.
  - **Step 3 — On-Card Profile Routing (msprof):** When msprof data is available, check the msprof routing table for `op_summary` field conditions and timeline signals. This step uses a different data source from Step 2.
- **Routing outcome:** After following the applicable steps, you should have 1-3 candidate patterns. Read only the one or two most relevant detailed pattern files under [`../triton-npu-optimize-knowledge/references/patterns/`](../triton-npu-optimize-knowledge/references/patterns/) to deepen the diagnosis. Record the routing result (which step matched, which conditions fired, final candidate list) in `attempts.md`. When neither Step 2 nor Step 3 data is available, use the `## High Priority Patterns` and `## Generated Pattern Summaries` sections in `pattern_index.md` as the normal fallback.
