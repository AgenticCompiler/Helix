...
- Generate missing correctness tests with `triton-npu-gen-test` only when the workspace does not already contain a usable harness.
- Generate missing benchmark cases with `triton-npu-gen-bench` only when the workspace does not already contain a usable harness.
...
- Use the sibling `triton-npu-analyze-ir` skill when compiler lowering details, stage-to-stage IR changes, or round-local IR evidence are needed to explain benchmark behavior. In optimize rounds, keep that evidence under `opt-round-N/ir/`, for example:
  ```bash
...
2. Generate missing tests or benchmarks through `../triton-npu-run-eval/scripts/run-command.py` before starting any optimization round.
...
22. Run the sibling `triton-npu-optimize-check` skill with `check-round` and repair the current round until it passes before starting another round or stopping.
...
- Always run `triton-npu-optimize-check check-baseline` before treating baseline preparation as complete.
