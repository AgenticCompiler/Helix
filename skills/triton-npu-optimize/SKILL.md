...
- During pattern triage, check for `extracted_bin_data/report.txt` relative to the operator workspace root and the current `opt-round-N/` directory. If found, read it before selecting patterns. `report.txt` only exists in simulation mode; when it is absent, skip Step 2 and use Step 1 plus the normal pattern index flow instead.
- **Pattern routing via `pattern_index.md`:** Read [`../triton-npu-optimize-knowledge/references/pattern_index.md`](../triton-npu-optimize-knowledge/references/pattern_index.md) as the primary routing entry point. It contains a `## Signal-Based Pattern Routing` section with three ordered steps. Step 1 is the primary routing step; Steps 2 and 3 provide supplementary diagnostic evidence:
  - **Step 1 — Code Structure Priority:** Always execute. Check code features against the table. Each row lists a code feature, candidate patterns, and differentiation criteria. This step requires no profiling data. If a code feature matches, record the candidate pattern(s). This step determines the primary pattern direction.
  - **Step 2 — Simulation Signal Routing (`extracted_bin_data/report.txt`):** Execute **only** when `extracted_bin_data/report.txt` exists and has been read. This file is present only in simulation mode. When `report.txt` is available, collect canonical features from all relevant sections **after** Step 1 has identified primary candidates, using the simulation data as supplementary diagnostic evidence to confirm severity and discover additional issues. Collect sections in order (`[Pipeline Flows]` → `[MTE2 Data Transport]` → `[WAIT_FLAG / BAR Sync]` → `[Pipe Distribution]` → `[Key Ratios]` → `[Pipe Overlap Ratio]` → `[VECTOR Unit]` → `[SCALAR Instr Types]` → `[TRACE Events]` → `[Simulator Runtime]` and available per-core sections) into one feature map, then evaluate Step 2 in two layers: first scan the section-organized signal tables, then evaluate the `Composite Match Rules` against the complete feature map. Signal-table rows marked as support for a composite route are supporting evidence only; they do **not** produce a pattern candidate until the corresponding composite rule's source gate, required signals, and reject conditions are checked. If conditional `[CUBE/MMA]` data exists, use it only as extra CUBE diagnostic detail, not as a separate routing entry point. This collect-then-match flow is required for cross-section conditions such as `CUBE_present = false` plus `TRACE_MADD_count > 0`; CUBE absence only routes to `classic-matmul` when the `classic-matmul` composite rule is satisfied, including the matmul-like source gate. Record which canonical features were observed, which signal-table rows matched, which composite rules were evaluated/fired/rejected, and what the simulation data reveals about the severity of Step 1 candidates or additional issues in `attempts.md`. When `report.txt` is absent, skip this step entirely.
  - **Step 3 — On-Card Profile Routing (msprof):** When msprof data is available, check the msprof routing table for `op_summary` field conditions and timeline signals. This step uses a different data source from Step 2 and similarly provides supplementary diagnostic evidence.
- **Routing outcome:** Combine primary candidates from Step 1 with supplementary severity evidence and additional findings from Step 2/3. You should have 1-3 candidate patterns. Read only the one or two most relevant detailed pattern files under [`../triton-npu-optimize-knowledge/references/patterns/`](../triton-npu-optimize-knowledge/references/patterns/) to deepen the diagnosis. When neither Step 2 nor Step 3 data is available, rely on Step 1 candidates plus the `## High Priority Patterns` and `## Generated Pattern Summaries` sections in `pattern_index.md`.
- **Routing traceability in `attempts.md`:** Record the full routing lookup process so that every pattern selection can be traced back to the routing step and condition that produced it. Do not jump directly to "Pattern candidate: X" without showing how you arrived there. The `attempts.md` entry must include:
  1. Which routing Steps were executed and why (e.g., "Step 1: always executed; Step 2: skipped — no `extracted_bin_data/report.txt` found; Step 3: skipped — no msprof data").
  2. For each executed Step, the routing flow within that Step:
     - **Step 1:** which code features were observed that triggered a match, and the corresponding candidate patterns from the routing table row (e.g., "Code feature: `tl.static_range` in hot loop → routing table row matched → candidate: `static-range-to-range`").
     - **Step 2:** which report.txt sections were scanned to collect canonical features, the observed feature map values used by routing, which signal-table rows matched, which matched rows were direct candidates versus support-only rows, which composite match rules were evaluated, and which candidate patterns were produced. For composite routes, record the source gate, required signals, supporting signals, and any reject conditions checked (e.g., "`CUBE_present = false` from `[Pipe Distribution]` and `TRACE_MADD_count = 48` from `[TRACE Events]` supported `classic-matmul`; source gate `matmul-like` passed; reject `already uses tl.dot` did not apply; composite rule fired").
     - **Step 3:** which msprof routing table rows matched, the observed `op_summary` field values, and the candidate patterns produced.
  3. The final candidate list produced by the routing process, before any personal knowledge override.
  4. If you supplement or override the routing result with your own optimization knowledge, state that explicitly as a separate decision after the routing trace.
