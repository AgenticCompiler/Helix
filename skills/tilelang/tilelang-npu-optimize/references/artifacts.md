...
Only add an entry when it is evidence-backed, portable to related TileLang Ascend NPU operators, and written as a reusable rule, diagnostic mapping, or optimization heuristic. Each entry should state where the lesson applies or what limits it.
...
- new TileLang code patterns that suggest an optimization opportunity
