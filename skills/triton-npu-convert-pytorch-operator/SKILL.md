...
5. Use `triton-npu-run-eval` to execute validation. For differential mode, run:
   ```bash
   python3 .opencode/skills/triton-npu-run-eval/scripts/run-command.py run-test-optimize \
     --test-file <test_file> \
     --operator-file <converted_output> \
     --test-mode differential \
     --baseline-operator-file <original_operator> \
     --compare-level balanced
   ```
   For standalone mode, run:
   ```bash
   python3 .opencode/skills/triton-npu-run-eval/scripts/run-command.py run-test-baseline \
     --test-file <test_file> \
     --operator-file <converted_output> \
     --test-mode standalone
   ```
   The command output must contain `PASS:` for validation to be considered successful. See "Validation Enforcement Rules" below for the complete set of mandatory validation constraints.
...
You MUST use the exact commands prescribed in "Validation Flow" step 5 above. Specifically:

- **Differential mode**: `run-command.py run-test-optimize` with `--baseline-operator-file <original> --compare-level balanced`
...
| Using `torch.allclose` / `torch.testing.assert_close` with custom tolerances | `torch.allclose(a, b, rtol=1e-5, atol=1e-8)` | Tolerances different from `balanced (rtol=1e-4, atol=1e-5)` may mask real errors or produce false positives |
...
- The literal output `PASS: ordered outputs match (level=balanced, rtol=0.0001, atol=1e-05)` — conversion is definitively validated
- The literal output `PASS: All tests passed!` or matching `All tests passed!` in the command output — standalone mode has passed
