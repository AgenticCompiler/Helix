...
def extract_pull_request_id_from_subject(subject: str) -> int | None:
...
def parse_pull_request_ids(values: list[str] | None) -> set[int]:
    ids: set[int] = set()
...
def resolve_pull_request_filter(
    ...
    knowledge_text: str,
) -> tuple[set[int] | None, Literal["cli", "knowledge-base", "none"]]:
    if explicit_ids:
