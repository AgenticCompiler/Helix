...
| **Copy `fla.*` under `deps/` (fallback)** | Only when import smoke fails after repo-path bootstrap, or when you need an isolated snapshot (`--copy-deps`). Copies **whole repo modules** needed for imports into `deps/fla/` (other kernels in those files may remain). `optimize-batch` optimizes **only** the root operator `.py` — do not edit or “trim” `deps/` by hand; keep the operator extract minimal instead. |
