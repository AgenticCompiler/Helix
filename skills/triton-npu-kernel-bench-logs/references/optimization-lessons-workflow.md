# From bench logs to `triton-npu-optimize-knowledge-v2` pattern cards

Use this workflow when distilling `workspace/NPUKernelBench_level_1_2_triton` (or a sibling export) into durable pattern knowledge. **Target only** `skills/triton-npu-optimize-knowledge-v2/` for pattern edits; leave `skills/triton-npu-optimize/` unchanged unless the project explicitly promotes the knowledge-v2 path.
...
2. **Guided round walk**
   ...
   - For each round, read `skills/triton-npu-optimize-knowledge-v2/references/pattern_index.md` and choose the best semantic match (one primary pattern and optional secondary pattern).
...
5. **Per-round narrative updates (`triton-npu-optimize-knowledge-v2` only)**
...
6. **Pattern card updates and new pattern creation**
   ...
   - If the round fits an existing card under `skills/triton-npu-optimize-knowledge-v2/references/patterns/`, update that card with the narrative and any refined guidance.
   - If the round is a "new pattern candidate", create `skills/triton-npu-optimize-knowledge-v2/references/patterns/<slug>.md` following `docs/notes/2026-04-29-optimize-pattern-card-authoring.md` (`# Title`, required `## Summary` and `## Use When`, optional sections as needed), then attach the round narrative there.
   - After all targeted pattern synthesis is complete, rewrite `skills/triton-npu-optimize-knowledge-v2/references/pattern_index.md` manually as a compact quick-match index (one `##` section per pattern, concise 4-5 line actionable summary, emphasis on when to use).
