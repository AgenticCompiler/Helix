...
- Scope kernels with optimization records (`opt-note.md` plus `opt-round-*`) and map rounds semantically against `skills/triton-npu-optimize-v2/references/pattern_index.md` (not keyword matching).
...
- For each mapped round, append an **expanded narrative** to the matched card in `skills/triton-npu-optimize-v2/references/patterns/`.
