...
When the task is to fold bench history into durable optimize knowledge (use **`skills/triton-npu-optimize-knowledge-v3/`** when that is the batch target instead of v2):
...
- Scope kernels with optimization records (`opt-note.md` plus `opt-round-*`) and map **every** round **semantically** against `skills/triton-npu-optimize-knowledge-v2/references/pattern_index.md` (not keyword matching). When the task targets **`triton-npu-optimize-knowledge-v3`**, use **`skills/triton-npu-optimize-knowledge-v3/references/pattern_index.md`** instead.
...
- For each mapped round, append an **expanded narrative** to the **one primary** pattern card under the active knowledge tree (`skills/triton-npu-optimize-knowledge-v2/references/patterns/` or **`skills/triton-npu-optimize-knowledge-v3/references/patterns/`** when that is the target).
...
- If unsure about style/shape, compare representative cards:
  - pre-synthesis style: `skills/triton-npu-optimize-knowledge/references/patterns/`
  - post-synthesis style: `skills/triton-npu-optimize-knowledge-v2/references/patterns/`

### 5) Rewrite `pattern_index.md` manually after full synthesis

- After synthesis is complete for all targeted pattern cards, rewrite `pattern_index.md` under the same knowledge tree you edited (`…-knowledge-v2` or **`…-knowledge-v3`**) as a human-authored quick-match index.
- Format requirements:
