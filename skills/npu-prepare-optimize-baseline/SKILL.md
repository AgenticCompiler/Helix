---
name: npu-prepare-optimize-baseline
description: Establish a reusable canonical optimize baseline by reusing or generating harnesses, performing minimum repair, and passing the submit-baseline skill.
---
...
- If a usable correctness harness is missing, use the sibling `npu-gen-test` skill.
- If a usable benchmark harness is missing, use the sibling `npu-gen-bench` skill.

### 2. Reach A Benchmarkable Start

- Use the sibling `npu-run-eval` skill for correctness validation and benchmark validation.
...
- Read the staged optimize skill's `references/artifacts.md` before writing `baseline/state.json`.
- Create `baseline/`.
...
- Use the sibling `the staged submit-baseline skill` skill to submit the baseline and validate it.
...
- `the staged submit-baseline skill` passes
