...
def _build_standalone_msprof_wrapper_script() -> str:
    return (
        ...
        "if not matching:\n"
        "    available = [case.get('id') for case in raw_cases if isinstance(case, dict)]\n"
        "    raise SystemExit(f'case_id {case_id!r} not found, available: {available!r}')\n"
        "case_fn = build_case_fn(operator_api, matching[0])\n"
        "if not callable(case_fn):\n"
        "    raise SystemExit(f'case_id {case_id!r} did not produce a callable benchmark function')\n"
        "print(f'[standalone-msprof] case={case_id} repeats={6}', "
        "      flush=True)\n"
        "for i in range(6):\n"
        "    case_fn()\n"
