...
- In `msprof-simulator` mode, `run-bench` executes `msprof op simulator python3 <test_file> --operator-file <operator_file>`. This mode requires `--test-file` to be passed in addition to `--bench-file` and `--operator-file`. The `--bench-file` is used for kernel resolution metadata, while `--test-file` is the actual script executed by the simulator.
...
python3 ./scripts/run-command.py run-bench --bench-file bench_<operator>.py --operator-file opt_<operator>.py --bench-mode msprof-simulator --test-file <test_file>.py
```
