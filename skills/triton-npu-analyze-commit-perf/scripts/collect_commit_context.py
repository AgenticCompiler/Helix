...
def list_commits(*, repo: Path, base_revision: str) -> list[dict[str, JsonValue]]:
    record_delimiter = f"\x00{COMMIT_RECORD_END}"
    output = run_git(
        [
            "log",
            f"{base_revision}..HEAD",
            "--reverse",
            f"--format=%H%x00%s%x00%b{record_delimiter}",
        ],
        cwd=repo,
    )
    commits: list[dict[str, JsonValue]] = []
    for block in output.split(record_delimiter):
        block = block.strip()
        if not block:
            continue
        if "\x00" not in block:
...
def git_show_file(*, repo: Path, sha: str, path: str) -> str | None:
