...
Generate a {Language} NPU operator benchmark script for one operator implementation.
...
- The supported entrypoint kinds are:
  - {Language} kernel-wrapper: a Python wrapper function that calls {Language} kernel functions
  - `torch-function`: a plain PyTorch-facing function or operator entrypoint that may internally call {Language} kernels
...
- When a `class Model` (or equivalent `torch.nn.Module`) calls a wrapper function and that wrapper launches the {Language} kernel, prefer the module class as the public entrypoint rather than selecting the intermediate wrapper function.
...
Use the npu-run-eval skill to execute generated benchmark cases.
...
1. Read the operator file and resolve the public entrypoint, `api-kind`, one or more {Language} kernel names, and realistic benchmark inputs.
...
6. If validation fails, repair only the benchmark file according to the self-repair rules below, retry, and then return a runnable script plus a short assumptions summary.
   - For {Language} Ascend compile, JIT, launch, or kernel-side failures, consult the `{language}-npu-repair-guide` skill as a diagnostic reference before deciding on the smallest safe benchmark-side change.
   - This workflow still owns only the generated benchmark file. Do not treat `{language}-npu-repair-guide` as permission to edit the operator file here.
...
- For `msprof`, fail explicitly if the public entrypoint is usable but the {Language} kernel names cannot be resolved safely.
...
| **Compiler error** ({Language} Ascend toolchain) | Use `{language}-npu-repair-guide` to help classify whether the symptom is harness-induced, then regenerate a fresh benchmark for the same operator and mode rather than patching line by line. If the failure is clearly operator-side, stop and report it. |
