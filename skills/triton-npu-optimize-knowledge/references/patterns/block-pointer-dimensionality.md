...
1. **High SCALAR-to-VECTOR ratio** — `%(SCALAR) / %(VECTOR) > 10`. Flat 1D pointer arithmetic generates heavy scalar address computation (`//`, `%`, `* stride`) that dwarfs useful vector work. Multidimensional `make_block_ptr` replaces scalar address generation with DMA descriptors, sharply reducing SCALAR.

2. **High MTE3 with SCALAR–MTE3 serialization** — `%(MTE3) > 10%` AND `%(SCALAR&MTE3/SCALAR) > 20%`. High MTE3 plus high SCALAR–MTE3 overlap means scalar address computation and data transfer are serialized. Block_ptr eliminates this by letting the DMA engine handle addressing autonomously.
...
5. **Low SCALAR–VECTOR overlap with high SCALAR** — `%(SCALAR&VECTOR/SCALAR) < 2%` AND `%(SCALAR) > 10%`. Scalar address generation is blocking vector execution. After block_ptr, SCALAR&VECTOR/SCALAR typically rises as the two run more concurrently.
...
3. **MTE2 is negligible** — `%(MTE2) < 0.5%` means memory access volume is too small to be the bottleneck; the issue is likely compute-bound or control-bound instead.
