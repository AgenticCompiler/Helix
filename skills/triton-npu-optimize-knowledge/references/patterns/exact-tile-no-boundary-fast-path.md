...
@triton.jit
def _kernel_unmasked(ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr, ...):
    ...  # no mask, no other, no autotune

@triton.autotune(configs=[...], key=["n_elements"])
@triton.jit
def _kernel_masked(ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr, ...):
    ...  # mask, other, autotune as before

def host_dispatch(ptr, out_ptr, n_elements, ...):
    chosen_block_size = BLOCK_SIZE_FIXED  # e.g. 8192
    if n_elements % chosen_block_size == 0:
        grid = (triton.cdiv(n_elements, chosen_block_size),)
        _kernel_unmasked[grid](..., BLOCK_SIZE=chosen_block_size)
    else:
        _kernel_masked[grid](...)  # autotune selects BLOCK_SIZE
```
...
NPUKernelBench `17_AdamW` used the 1D unmasked fast path on 18 elementwise shapes. For exact-tile shapes (`n_elements % 8192 == 0`), kernel latency dropped up to **21% on small shapes** and achieved **1.26x overall geomean speedup** vs masked-only baseline.
...
- `compile_hint`
- `tiling` (choosing BLOCK_SIZE to maximize exact-tile coverage)
- `autotune` (composes: remove autotune from unmasked path, keep on masked fallback)
- `padded_row_col_copy`
