...
- For each mapped round, append an **expanded narrative** to the **one primary** pattern card under the active staged knowledge skill (`../<Language>-npu-optimize-knowledge/references/patterns/`, which the staging system maps from whichever v2/v3 source tree is the target).
...
- If unsure about style/shape, compare representative cards:
  ...
  - post-synthesis style: the pattern cards you are actively editing in this workflow (same staged skill, `../<Language>-npu-optimize-knowledge/references/patterns/`)

### 5) Rewrite `pattern_index.md` manually after full synthesis

- After synthesis is complete for all targeted pattern cards, rewrite `pattern_index.md` under the staged knowledge skill (`../<Language>-npu-optimize-knowledge/references/pattern_index.md`) as a human-authored quick-match index.
- Format requirements:
