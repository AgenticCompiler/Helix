...
- `metric_source` selects how `compare-perf` derives each case's timing: `auto` preserves the current kernel-first fallback behavior, `kernel` requires kernel latency, `total-op` requires total-op aggregation (from raw op statistics in profiler modes, or derived from `kernel_avg_time_us` in `perf-counter` mode), and `all` prints both kernel and total-op comparison sections.
