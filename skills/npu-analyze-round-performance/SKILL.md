---
name: npu-analyze-round-performance
...
This skill is for deep round analysis inside `{language}-npu-optimize`, not for supervisor audits and not for whole-session summaries.
This skill is the owner of `opt-round-N/perf-analysis.md` for round-level performance diagnosis inside `{language}-npu-optimize`.
...
When IR attribution is needed, use `{language}-npu-analyze-ir` as the IR evidence companion for capture, navigation, and stage-level inspection. That does not transfer ownership of `opt-round-N/perf-analysis.md`.
...
When compiler source analysis is enabled by the launch prompt or workspace guidance, treat it as a later escalation after profile and IR analysis. Use `{language}-npu-analyze-compiler-source` only when this skill has narrowed the problem to a concrete performance-related compiler-side question that still needs source-backed explanation before the next operator change is clear.
...
Read [`../{language}-npu-optimize-knowledge/references/symptom_index.md`](../{language}-npu-optimize-knowledge/references/symptom_index.md) when structured profile or IR evidence is available and you need {Language}/kernel-oriented symptom cards to narrow likely pattern directions before returning to detailed pattern references.
...
2. Confirm the round has profile evidence.
   ...
   - If profile evidence is missing, collect it first through the existing profiling flow from [`../npu-profile-operator/SKILL.md`](../npu-profile-operator/SKILL.md).
...
4. Extract profile signals first.
   - Prefer the bundled summary helper in JSON mode:
     ```bash
      python3 ../npu-run-eval/scripts/run-command.py profile-report --profile-dir <profile-dir> --format json
     ```
...
6. Use [`../{language}-npu-optimize-knowledge/references/symptom_index.md`](../{language}-npu-optimize-knowledge/references/symptom_index.md) and the matching symptom cards to narrow the current hypothesis.
   - Start from the symptom index, then read only the one or two symptom cards under [`../{language}-npu-optimize-knowledge/references/symptoms/`](../{language}-npu-optimize-knowledge/references/symptoms/) that best match the extracted evidence.
...
8. Extract IR performance signals as the second analysis path for explanation and attribution.
   - Prefer:
     ```bash
     python3 ../{language}-npu-analyze-ir/scripts/inspect_ir.py performance-signals --ir-dir <ir-dir> --format json
     ```
...
- Keep artifact ownership here even when IR evidence is used; `{language}-npu-analyze-ir` is the IR evidence companion, not the owner of `perf-analysis.md`.
