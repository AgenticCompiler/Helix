...
- Generate a correctness test via `npu-gen-test`.
- Generate a benchmark via `npu-gen-bench`.
- Validate both artifacts via `npu-run-eval`.
...
3. Ensure the operator.s {Language} path actually runs, and remove any code paths that fall back to PyTorch.
4. Generate the correctness test with the `npu-gen-test` skill.
5. Validate the generated test with the `npu-run-eval` workflow, using the generated file against the current operator file.
...
8. Generate the benchmark with the `npu-gen-bench` skill.
9. Validate the generated benchmark with the `npu-run-eval` workflow.
...
## Repair experience ({Language} Ascend)

When steps 2, 6, or 10 require **operator-side** fixes for compile / JIT / kernel errors on Ascend, use the `{language}-npu-repair-guide` skill and read [../{language}-npu-repair-guide/references/repair-experience.md](../{language}-npu-repair-guide/references/repair-experience.md) for team-maintained heuristics. These hints do not override `npu-gen-test`, `npu-gen-bench`, or normative specs.

If the successful fix is a new pattern not covered there, append a short entry to [../{language}-npu-repair-guide/output.md](../{language}-npu-repair-guide/output.md).

## Validation Commands

- Use the `npu-run-eval` skill for correctness validation, with `python3 ../npu-run-eval/scripts/run-command.py run-test-baseline ...` as the standard helper command.
- Use the `npu-run-eval` skill for benchmark validation, with `python3 ../npu-run-eval/scripts/run-command.py run-bench ...` as the standard helper command.
...
- Keep generated test and benchmark files aligned with the final operator API. They must follow **`npu-gen-test` / `npu-gen-bench`** norms: harnesses **run on Ascend NPU only**—no CUDA/CPU/other-device primary paths (see those skills’ specs).
...
- When editing the operator, **preserve the {Language} Ascend NPU kernel path** as the delivered implementation. **Do not** replace it with a **pure PyTorch** reimplementation as a “fix,” and **do not** satisfy validation by weakening what the harness exercises.

## Do Not

- Do not mask operator failures in harnesses. Never add try/except, alternate paths, shims, or PyTorch-only fallbacks in generated tests/benchmarks to green `npu-run-eval` correctness-test / `run-bench` validations when the real issue is {Language} compile, launch, or operator logic—**forbidden**.
...
- Do not use the `{language}-npu-optimize` skill for this workflow.
