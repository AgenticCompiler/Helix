...
Use the `npu-run-eval` skill to validate generated tests.
...
4. If validation fails, repair the test and repeat.
   - For {Language} Ascend compile, JIT, launch, or kernel-side failures, consult the `{language}-npu-repair-guide` skill as a diagnostic reference before deciding on the smallest safe test-side change.
   - This workflow still owns only the generated test file. Do not treat `{language}-npu-repair-guide` as permission to edit the operator file here.
...
| **Compiler error** ({Language} Ascend toolchain) | Use `{language}-npu-repair-guide` to help classify whether the symptom is harness-induced, then regenerate a fresh test for the same operator and mode rather than patching line by line. If the failure is clearly operator-side, stop and report it. |
